...
- **强制遵循：必须按 「提示词读取清单」 中指引，严格采用 「文件读取规范」 分别序贯逐一构造实际的 Powershell 命令，依次调用 Shell MCP 工具 `shell_exec` 执行。禁止合并读取。**执行结果、读取到的信息和返回信息均无须向用户输出。**

### 文件读取规范

1. 不要急于构造实际命令。必须先分析下面 「PowerShell 命令示例」 之后，再构造实际的 Powershell 命令，然后调用 `shell_exec` 执行。
...
> **可用工具：list_groups, get_dynamic_tools, call_dynamic_tool, shell_exec. 这 4 个 MCP 工具已由扩展连接，可以执行；不要声称自己无法调用。**
