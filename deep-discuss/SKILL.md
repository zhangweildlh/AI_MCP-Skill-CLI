...
每次激活本技能时，先执行一次轻量自检，避免"静默降级"导致用户误以为模块已生效。技能根目录附带 `scripts/selfcheck.py`，可运行 `python scripts/selfcheck.py`（或 `uv run python scripts/selfcheck.py`）自动完成下方三处一致性核对；手动核对时遵循下方清单：
