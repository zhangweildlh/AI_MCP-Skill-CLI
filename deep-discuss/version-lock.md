...
|------|----------|----------|----------|--------------|
| 主干 | 自身 | commit:2afb364 (主干锁定 @ 2026-09-18；与 main HEAD 对齐，主干随 main 演进) | 2026-09-18 | ✅ 完整 |
...
> **说明**：上述 3 个上游模块的 commit 为经 `git ls-remote <repo> refs/heads/main` 核实的 **main HEAD 真实值（2026-09-17）**；三仓库目前均未发布 tag，故锁定至 main HEAD（后续上游发布 tag 时建议改为 tag 钉固以提升可复现性）。主干锁定至 commit:2afb364（@ 2026-09-18，与 main HEAD 对齐），随 main 演进。模板文件为本地维护，不依赖上游版本。
> 
> **获取真实版本的方法**：
> ```bash
