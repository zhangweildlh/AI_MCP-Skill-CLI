...
### [产出] ★（必须写入的精确文件清单）
- **置顶**列出本步必须生成 / 更新的文件及精确路径（`[输出目录]/文件名`）。
- 未产出则本步视为未完成。
...
### [验证] ★（FILE_STAT 验证闸门）
- 使用 `FILE_STAT` 原语确认 [产出] 中列出的文件**确实存在**。
- 不存在 → 用 `WRITE_FILE` 补写后重新校验。
- `FILE_STAT` 不可用时，使用状态文件映射表中的等价替代（如 SHELL `Test-Path` / `test -f`）。
...
本契约在六段之外新增"续跑"维度，应对 LLM 会话限额导致的新开会话接力续跑场景：
