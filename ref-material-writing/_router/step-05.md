...
> rmw 内部步骤5。遵循 `_router/_contract.md` 七段契约。
...
- **引用 `references/16-self-check-D.md` 标准档**校验：存在性 + 非空阈值 + 轻签名（字节/条目数 vs [产出] 预期）+ 结构完整性（每关键词双引擎段齐全、来源条数 ≥ `min_independent_sources_per_claim`、含 `[资料X]/[搜索X]` 溯源标记）。
- **向 §14 写入登记（设计点 5）**：通过则写入"✅通过 + 轻签名 + 登记对象=`[输出目录]/_网络搜索素材.md` + 子阶段=关键词级（如 K3-Firecrawl done）+ 自检时刻"；失败回退补跑缺失引擎/关键词后重检。
