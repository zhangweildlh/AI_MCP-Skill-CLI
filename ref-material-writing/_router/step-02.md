...
| URL | **双轨抓取（并行 AnySearch）**：① 调用 WEB_FETCH（Firecrawl 形态见映射表：直连 `firecrawl_scrape` / 中继 `mcp__Dynamic-mcp__call_dynamic_tool(name="firecrawl_scrape")`）获取网页文本；② 同时调用 AnySearch `extract <url>` 补充抓取全文（命令：`<ANYSEARCH_CMD> extract "<url>"`；`<ANYSEARCH_CMD>` 唯一定义于 `references/13-anysearch-integration.md` §1，调用前先读该节取完整前缀）。两路结果合并入同一分析卡片，互不替代。 |
