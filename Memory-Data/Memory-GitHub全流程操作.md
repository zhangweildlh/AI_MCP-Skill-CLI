...
| 适用场景 | 章节位置 | 备注 |
|---------|---------|------|
| 3-1 git/gh 工具与认证 | `## 3-1 git/gh 工具与认证` |  |
| 3-1-1 git / gh 本地路径 | `### 3-1-1 git / gh 本地路径` |  |
| 3-1-2 凭证助手桥接 | `### 3-1-2 凭证助手桥接` |  |
| 3-1-3 连接器说明 | `### 3-1-3 连接器说明` |  |
| 3-1-4 能力边界（重要） | `### 3-1-4 能力边界（重要）` |  |
| 3-2 账户信息 | `## 3-2 账户信息` |  |
| 3-2-1 用户名与邮箱 | `### 3-2-1 用户名与邮箱` |  |
| 3-2-2 Fork 仓库清单（账户资产索引） | `### 3-2-2 Fork 仓库清单（账户资产索引）` |  |
| 3-3 gh 能力速查 | `## 3-3 gh 能力速查` |  |
| 3-3-1 认证与配置 | `### 3-3-1 认证与配置` |  |
| 3-3-2 仓库管理（《gh repo》） | `### 3-3-2 仓库管理（《gh repo》）` |  |
| 3-3-3 Pull Request（《gh pr》） | `### 3-3-3 Pull Request（《gh pr》）` |  |
| 3-3-4 Issue 跟踪（《gh issue》） | `### 3-3-4 Issue 跟踪（《gh issue》）` |  |
| 3-3-5 全站搜索（《gh search》） | `### 3-3-5 全站搜索（《gh search》）` |  |
| 3-3-6 原生 API 访问（《gh api》） | `### 3-3-6 原生 API 访问（《gh api》）` |  |
| 3-3-7 CI/CD（《gh run》 / 《gh wor... | `### 3-3-7 CI/CD（《gh run》 / 《gh workflow》）` |  |
| 3-3-8 发布与制品（《gh release》） | `### 3-3-8 发布与制品（《gh release》）` |  |
| 3-3-9 代码片段（《gh gist》） | `### 3-3-9 代码片段（《gh gist》）` |  |
| 3-3-10 密钥与变量（《gh secret》 / 《gh... | `### 3-3-10 密钥与变量（《gh secret》 / 《gh variable》）` |  |
| 3-3-11 标签 / 项目 / 规则集 | `### 3-3-11 标签 / 项目 / 规则集` |  |
| 3-3-12 扩展与定制 | `### 3-3-12 扩展与定制` |  |
| 3-3-13 其他 | `### 3-3-13 其他` |  |
| 3-4 提交前文档同步门禁 | `## 3-4 提交前文档同步门禁` |  |
| 3-4-1 分层模型与处理语义 | `### 3-4-1 分层模型与处理语义` |  |
| 3-4-2 适用范围 | `### 3-4-2 适用范围` |  |
| 3-4-3 执行流程（查询 → 分析 → 需改先改 → 提交... | `### 3-4-3 执行流程（查询 → 分析 → 需改先改 → 提交 → 推送）` |  |
| 3-4-4 与现有规则关系 | `### 3-4-4 与现有规则关系` |  |
| 3-5 Fork CI 实证要点 | `## 3-5 Fork CI 实证要点` |  |
| 3-5-1 关键问题与解决 | `### 3-5-1 关键问题与解决` |  |
| 3-5-2 约束 / 注意事项（fork 专属硬规则） | `### 3-5-2 约束 / 注意事项（fork 专属硬规则）` |  |
| 3-6 工作流一 github-personal-manag... | `## 3-6 工作流一 github-personal-manager 自动激活（GitHub 工作流总闸门，前置）` |  |
| 3-6-1 激活判定 | `### 3-6-1 激活判定` |  |
| 3-6-2 激活动作 | `### 3-6-2 激活动作` |  |
| 3-6-3 能力覆盖范围 | `### 3-6-3 能力覆盖范围` |  |
| 3-6-4 与现有 GitHub 规则的关系 | `### 3-6-4 与现有 GitHub 规则的关系` |  |
| 3-6-5 门禁仍生效 | `### 3-6-5 门禁仍生效` |  |
| 3-6-6 激活后执行点索引 | `### 3-6-6 激活后执行点索引` |  |
| 3-7 工作流二 信息读取与搜索 | `## 3-7 工作流二 信息读取与搜索` |  |
| 3-7-1 适用范围 | `### 3-7-1 适用范围` |  |
| 3-7-2 优先顺序（硬规则） | `### 3-7-2 优先顺序（硬规则）` |  |
| 3-7-3 决策流 | `### 3-7-3 决策流` |  |
| 3-7-4 跨命令通用约束 | `### 3-7-4 跨命令通用约束` |  |
| 3-8 工作流三 日常同步巡检 | `## 3-8 工作流三 日常同步巡检` |  |
| 3-8-1 阶段 0 — 配置校验（前置门槛） | `### 3-8-1 阶段 0 — 配置校验（前置门槛）` |  |
| 3-8-2 第一步 — 本地 ↔ 你的远端仓库(origin... | `### 3-8-2 第一步 — 本地 ↔ 你的远端仓库(origin)` |  |
| 3-8-3 第二步 — 你的远端仓库(origin/fork... | `### 3-8-3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)` |  |
| 3-8-4 冲突处理（六类问题） | `### 3-8-4 冲突处理（六类问题）` |  |
| 3-8-5 强门禁总述 | `### 3-8-5 强门禁总述` |  |
| 3-9 工作流四 标准代码修改 | `## 3-9 工作流四 标准代码修改` |  |
| 3-9-1 阶段 0 启动前闸门 | `### 3-9-1 阶段 0 启动前闸门` |  |
| 3-9-2 阶段 1 同步与建分支 | `### 3-9-2 阶段 1 同步与建分支` |  |
| 3-9-3 阶段 2 提交 / 推送 / 触发 CI | `### 3-9-3 阶段 2 提交 / 推送 / 触发 CI` |  |
| 3-9-4 阶段 3 对齐上游并强推（功能分支(feat)，... | `### 3-9-4 阶段 3 对齐上游并强推（功能分支(feat)，非 main）` |  |
| 3-9-5 阶段 4 合并 | `### 3-9-5 阶段 4 合并` |  |
| 3-9-6 阶段 5 收尾同步与清理 | `### 3-9-6 阶段 5 收尾同步与清理` |  |
| 3-9-7 硬约束 | `### 3-9-7 硬约束` |  |
| 3-9-8 实用工具：stash 临时抽屉（被打断时保存进度... | `### 3-9-8 实用工具：stash 临时抽屉（被打断时保存进度）` |  |
| 3-10 工作流五 多工作树并行开发（–no-ff 普通合并... | `## 3-10 工作流五 多工作树并行开发（–no-ff 普通合并特化）` |  |
| 3-10-1 适用范围 | `### 3-10-1 适用范围` |  |
| 3-10-2 方法论（心智模型 · 为什么这样做） | `### 3-10-2 方法论（心智模型 · 为什么这样做）` |  |
| 3-10-3 四大约束 ↔ 唯一验证矩阵 | `### 3-10-3 四大约束 ↔ 唯一验证矩阵` |  |
| 3-10-4 核心原则（不可动摇） | `### 3-10-4 核心原则（不可动摇）` |  |
| 3-10-5 参数占位与前置条件 | `### 3-10-5 参数占位与前置条件` |  |
| 3-10-6 阶段一：从主线开出独立工作树（多任务并行） | `### 3-10-6 阶段一：从主线开出独立工作树（多任务并行）` |  |
| 3-10-7 阶段二：工作树内开发 + 测试 | `### 3-10-7 阶段二：工作树内开发 + 测试` |  |
| 3-10-8 阶段三：《--no-ff》 合并到主线(mai... | `### 3-10-8 阶段三：《--no-ff》 合并到主线(main)` |  |
| 3-10-9 阶段四：整段回滚能力验证（推荐，非强制但可选） | `### 3-10-9 阶段四：整段回滚能力验证（推荐，非强制但可选）` |  |
| 3-10-10 阶段五：清理工作树(worktree)（按要... | `### 3-10-10 阶段五：清理工作树(worktree)（按要求 / 条件）` |  |
| 3-10-11 阶段六：清理分支(branch)（本地 + ... | `### 3-10-11 阶段六：清理分支(branch)（本地 + 远端，按要求 / 条件）` |  |
| 3-10-12 硬禁令与红线（唯一，不可逾越） | `### 3-10-12 硬禁令与红线（唯一，不可逾越）` |  |
| 3-10-13 验收闸门清单（四张，逐项打勾） | `### 3-10-13 验收闸门清单（四张，逐项打勾）` |  |
| 3-10-14 完整命令清单（可复制模板） | `### 3-10-14 完整命令清单（可复制模板）` |  |
| 3-10-15 常见问题与唯一处置（通用化） | `### 3-10-15 常见问题与唯一处置（通用化）` |  |
| 3-10-16 风险与反模式 | `### 3-10-16 风险与反模式` |  |
| 3-10-17 一句话方法论 | `### 3-10-17 一句话方法论` |  |
| 3-11 工作流六 CI 失败排错 | `## 3-11 工作流六 CI 失败排错` |  |
| 3-11-1 第一步 — 定位失败的 run 与 job | `### 3-11-1 第一步 — 定位失败的 run 与 job` |  |
| 3-11-2 第二步 — 按类型对号入座（详见本模块对应条目... | `### 3-11-2 第二步 — 按类型对号入座（详见本模块对应条目）` |  |
| 3-11-3 第三步 — 修复回推与重跑 | `### 3-11-3 第三步 — 修复回推与重跑` |  |
| 3-12 工作流七 Release 发版 | `## 3-12 工作流七 Release 发版` |  |
| 3-12-1 第一步 — 发版前检查（缺一不可） | `### 3-12-1 第一步 — 发版前检查（缺一不可）` |  |
| 3-12-2 第二步 — 打标签(tag) 触发（公开动作，... | `### 3-12-2 第二步 — 打标签(tag) 触发（公开动作，暂停等指令）` |  |
| 3-12-3 第三步 — 监控与取产物 | `### 3-12-3 第三步 — 监控与取产物` |  |
| 3-13 工作流八 分支清理回收 | `## 3-13 工作流八 分支清理回收` |  |
| 3-13-1 第一步 — 识别可清理分支 | `### 3-13-1 第一步 — 识别可清理分支` |  |
| 3-13-2 第二步 — 清理（删除类动作，暂停等指令） | `### 3-13-2 第二步 — 清理（删除类动作，暂停等指令）` |  |
| 3-13-3 强门禁（删除专属） | `### 3-13-3 强门禁（删除专属）` |  |
| 3-13-4 工区内部对象回收（git gc / reflo... | `### 3-13-4 工区内部对象回收（git gc / reflog / 悬空提交）` |  |
| 3-14 工作流九 PR 全生命周期操作 | `## 3-14 工作流九 PR 全生命周期操作` |  |
| 3-14-1 阶段 0 — 前置门禁（引用，不重复定义） | `### 3-14-1 阶段 0 — 前置门禁（引用，不重复定义）` |  |
| 3-14-2 阶段 1 — 开新 PR 前的三核验 | `### 3-14-2 阶段 1 — 开新 PR 前的三核验` |  |
| 3-14-3 阶段 2 — 重复 PR 检查（引用 + PR... | `### 3-14-3 阶段 2 — 重复 PR 检查（引用 + PR 专属口径）` |  |
| 3-14-4 阶段 3 — 查询并遵循上游仓库对 PR 的要... | `### 3-14-4 阶段 3 — 查询并遵循上游仓库对 PR 的要求与规范（PR 专属，新内容）` |  |
| 3-14-5 阶段 4 — 开新 PR 的规范操作（引用 +... | `### 3-14-5 阶段 4 — 开新 PR 的规范操作（引用 + PR 专属补充）` |  |
| 3-14-6 阶段 5 — PR 审查意见回应（核心，含多轮... | `### 3-14-6 阶段 5 — PR 审查意见回应（核心，含多轮）（PR 专属，新内容）` |  |
| 3-14-7 阶段 6 — PR 合并（引用） | `### 3-14-7 阶段 6 — PR 合并（引用）` |  |
| 3-14-8 阶段 7 — 其他 PR 操作（引用 [`##... | `### 3-14-8 阶段 7 — 其他 PR 操作（引用 [`### 3-3-3 Pull Request（《gh pr》）`](#3-3-3-pull-requestgh-pr)，补用法）` |  |
| 3-14-9 阶段 8 — 收尾与清理（引用） | `### 3-14-9 阶段 8 — 收尾与清理（引用）` |  |
| 3-14-10 强门禁总述（PR 全生命周期） | `### 3-14-10 强门禁总述（PR 全生命周期）` |  |
| 3-15 工作流十 清理工区维护 | `## 3-15 工作流十 清理工区维护` |  |
| 3-15-1 可删除文件定义（清理工区时的目标集） | `### 3-15-1 可删除文件定义（清理工区时的目标集）` |  |
| 3-15-2 清理工区工作流（强门禁：先搜列、后确认、再删除... | `### 3-15-2 清理工区工作流（强门禁：先搜列、后确认、再删除）` |  |
| 3-15-3 必须保留与受保护清单（清理工区永不删） | `### 3-15-3 必须保留与受保护清单（清理工区永不删）` |  |
<!-- INDEX_END -->
## 3-1 git/gh 工具与认证
...
### 3-3-7 CI/CD（《gh run》 / 《gh workflow》）
...
### 3-3-10 密钥与变量（《gh secret》 / 《gh variable》）
...
| Tier 2（应处理） | docs/、CONTRIBUTING、配置样例、接口契约、i18n、examples | 存在但未纳入变更 → 提交(commit)前须处理，或显式说明为何不改；脚本加 --strict 可同样阻断（此层级=须处理或说明，非可跳过） |
...
3. **查仓库是否存在清单文件并分析同步状态**：运行 `bash scripts/sop_docs_sync_check.sh <仓库路径>`（技能内脚本，只读 dry-run，依据 `references/docs-sync-checklist.md` 数据块）——它查仓库是否存在清单文件、对存在文件核对是否已纳入本次变更，输出分层明细（`【Tier 1｜阻断】` / `【Tier 2｜应处理】` / `【Tier 3｜提示】`）与 `【文档同步状态】已同步 / 未同步`。
4. **需改则先改再提交(commit)**：
...
> 本模块记录 Fork 仓库 CI 实操中独有的坑与专属约束，作为 GitHub 标准工作流章节中 CI / 发版类（工作流六/七）实操坑的实证溯源；已抽离账户名/邮箱，**统一用占位符 `<upstream>`/`<fork>`/`<feat>`/`<version>` 与「某仓库/某版本」表述**，确保可移植。涉及具体语言的坑（Rust/Node 等）已标注适用语言，不具备跨语言通用性。各主流程工作流已以「详见模块·本章第 X 点」单向指向此处（指向格式以速查索引表第二列为准）。详细展开见 `github-personal-manager` 技能 `references/fork-ci-pitfalls.md`。
...
>
> 本规则作为 GitHub 工作流总闸门，逻辑上前置（位于路径核验等模块/工作流章节之前）。各工作流在启动前均须先激活本技能。
...
该技能是 GitHub 个人操作的统一执行入口，覆盖「信息读取与搜索 / 日常同步巡检 / 标准代码修改 / 多工作树并行开发 / CI 失败排错 / Release 发版 / 分支清理回收 / PR 全生命周期 / 清理工区」等全部 GitHub 工作流（其中自动激活为本总闸门自身）；凡上述场景优先走该技能，而非手工拼 git/gh 命令。**各工作流章节已统一标注"启动前必须先完成路径核验"，即本门禁的落地**。

### 3-6-4 与现有 GitHub 规则的关系
...
> 本索引与技能 `SKILL.md` 的「核心工作流」章节一一对应；记忆侧负责"约束与为什么"（全局禁令章节 + GitHub 标准工作流各章），技能侧负责"具体跑哪个脚本、怎么跑"，两者组合即可端到端落地任意 GitHub 操作。
...
4. `gh` 能力总览覆盖的任务/事项/操作（见 [`## 3-3 gh 能力速查`](#3-3-gh-能力速查)）。
...
满足才改用 WebFetch/WebSearch；否则，网页搜索应走 firecrawl-mcp（经 dynamic-mcp 聚合器）兜底。
...
- 双向分叉 → 暂停+候选方案（A 以你的远端仓库(origin) 为准 `git reset --hard origin/main` / B 以本地为准：经功能分支(feat) 走 PR 合并(merge) 后再同步（**禁止强推 main，见 [`## 1-2 全局禁令（最高优先级）`](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-全局禁令与环境约束.md#1-2-全局禁令最高优先级) **）/ C 合并(merge) 保留双方 / D 变基(rebase) / E 中止不动；疑似验证残留选 A，仍暂停等确认）。
- ⚠️ 选项 A 的 `git reset --hard origin/main` 仅当仓库**不含 `.workbuddy`** 或已备份、且获用户明确授权时方可；含 `.workbuddy` 的仓库按 [`### 1-2-4 Git 撤销/回退安全（reset 三模式 + 硬约束）`](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-全局禁令与环境约束.md#1-2-4-git-撤销回退安全reset-三模式--硬约束) 禁用 `reset --hard`（防误删项目记忆）。

### 3-8-3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)
...
- **有未合并也未拒绝的开放 PR（state=open）→ 记为「PR 待审」态**：大白话报告 PR 编号、base、状态，说明这是贡献回 上游仓库(upstream) 的正常通道；不重复开 PR、不覆盖、不暂停，继续巡检/结束；若同时 K>0（上游仓库(upstream) 已前进），额外提示「PR 可能落后于 上游仓库(upstream)」，当 K>0 时，应 rebase 功能分支(feat) 后更新 PR，并暂停等你指令。
...
- **问题一** 本地 main ↔ 你的远端仓库(origin) 双向分叉：A–E 方案，暂停等确认（见 [`### 3-8-2 第一步 — 本地 ↔ 你的远端仓库(origin)`](#3-8-2-第一步--本地--你的远端仓库origin) 双向分叉）
...
### 3-9-4 阶段 3 对齐上游并强推（功能分支(feat)，非 main）
...
- 重跑 CI 至绿
- ⚠️ **强推一律 `--force-with-lease`，禁用裸 `--force`**：`--force-with-lease` 推送前会检查远程是否有本地不知晓的更新（他人新提交），有则拒绝推送、避免覆盖他人工作；裸 `--force` 无条件覆盖，风险高。仅强推 feat 分支，绝不强推 main（见 [`### 1-2-1 禁止强推/删除自家 main（及受保护分支）`](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-全局禁令与环境约束.md#1-2-1-禁止强推删除自家-main及受保护分支)）。
...
### 3-9-8 实用工具：stash 临时抽屉（被打断时保存进度）
...
- 巡检（见 [`## 3-8 工作流三 日常同步巡检`](#3-8-工作流三-日常同步巡检)）遇工作区脏会硬停止——`stash` 是先"暂存半截"再继续的安全替代，但 stash 后仍需在合适时机 `pop` 回来继续。
...
4. **硬禁令（见 [`## 1-2 全局禁令（最高优先级）`](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-全局禁令与环境约束.md#1-2-全局禁令最高优先级)）**：禁止对 `<REMOTE>/<BASE_BRANCH>` 执行强推（`--force` / `-f` / `--force-with-lease`）或删除。
...
- `<ABS_PATH>`：绝对路径，应为 `<WORKTREE_ROOT>/<topic>`（独立父目录，与主仓库分离）。
...
**守约定**：项目最高优先级约定区域功能完全保留（见 [`### 3-10-4 核心原则（不可动摇）`](#3-10-4-核心原则不可动摇) 第5点）。
...
- **最高优先级约定区域冲突**（项目声明的核心模块）：**立即暂停，告知用户**，大白话说明，不擅自解（见 [`### 3-10-4 核心原则（不可动摇）`](#3-10-4-核心原则不可动摇) 第5点）。

4. `git add <file>` 逐个标记已解决。
5. `git commit`（合并继续；可 `--no-edit` 或补正文）。

**合并碑结构验证（必须全满足，对照 [`### 3-10-3 四大约束 ↔ 唯一验证矩阵`](#3-10-3-四大约束--唯一验证矩阵)）**：
...
> 注：提交(commit)前文档—代码同步仍须过 [`## 3-4 提交前文档同步门禁`](#3-4-提交前文档同步门禁)（Tier 1 未同步必须先补文档）；本节为合并后补充提交(commit)的专用要求。
...
- 无 `-f` / `--force` / `--force-with-lease`，合规，不触发硬禁令（见 [`## 1-2 全局禁令（最高优先级）`](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-全局禁令与环境约束.md#1-2-全局禁令最高优先级)）。触发仓库 CI（若有），须全绿。
...
### 3-10-10 阶段五：清理工作树(worktree)（按要求 / 条件）
...
### 3-10-11 阶段六：清理分支(branch)（本地 + 远端，按要求 / 条件）
...
### 3-10-12 硬禁令与红线（唯一，不可逾越）

- 禁止 `git push --force` / `-f` / `--force-with-lease <REMOTE> <BASE_BRANCH>`（见 [`## 1-2 全局禁令（最高优先级）`](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-全局禁令与环境约束.md#1-2-全局禁令最高优先级)）。
...
### 3-10-13 验收闸门清单（四张，逐项打勾）

**合并前（ [`### 3-10-8 阶段三：《--no-ff》 合并到主线(main)`](#3-10-8-阶段三--no-ff-合并到主线main) 预检）**：
...
**合并后（ [`### 3-10-8 阶段三：《--no-ff》 合并到主线(main)`](#3-10-8-阶段三--no-ff-合并到主线main) 合并碑验证 + 合并后验证）**：

- [ ] 合并碑双父（`grep -c parent` = 2）
- [ ] `<TIP>` 是 HEAD 祖先（返回 0）
- [ ] 功能中间提交(commit)可见（`git log HEAD^2`）
- [ ] 四大约束验证矩阵（ [`### 3-10-3 四大约束 ↔ 唯一验证矩阵`](#3-10-3-四大约束--唯一验证矩阵)）全满足
- [ ] 变更文档(CHANGELOG)已补合并说明 + 回滚路径
- [ ] 版本号一致（若发版）
- [ ] `git push <REMOTE> <BASE_BRANCH>` 成功，CI 全绿

**清理工作树前（ [`### 3-10-10 阶段五：清理工作树(worktree)（按要求 / 条件）`](#3-10-10-阶段五清理工作树worktree按要求--条件)）**：

- [ ] 代码已合并（`merge-base --is-ancestor <TIP> HEAD` = 0）
- [ ] 工作树(worktree)无独特未跟踪产物（或已备份）
- [ ] `git worktree list` 确认活跃 / 游离状态，选对删除命令

**清理分支前（ [`### 3-10-11 阶段六：清理分支(branch)（本地 + 远端，按要求 / 条件）`](#3-10-11-阶段六清理分支branch本地--远端按要求--条件)）**：
...
### 3-10-14 完整命令清单（可复制模板）

> **兜底说明（脚本化封装）**：下列手写命令模板的等价、可复制执行封装，由 `github-personal-manager` 技能的 `scripts/sop_worktree_add.sh` / `scripts/sop_worktree_merge.sh` / `scripts/sop_worktree_cleanup.sh` 提供，已内置本清单全部安全约束（`--no-ff` 双父碑合并、冲突预测暂停、合并校验防丢提交、本地远程跟踪引用清理 `git branch -d -r`、dry-run 优先、脏区硬停）。**优先直接调用脚本**；本清单作为原理参考与脚本不可用时的兜底，所有脚本约束与 [`### 3-6-6 激活后执行点索引`](#3-6-6-激活后执行点索引) 索引一致。
...
### 3-11-2 第二步 — 按类型对号入座（详见本模块对应条目）

| 失败现象 | 根因与去处 | 修复方向 |
| ------ | ------ | ------ |
| cargo fmt – --check 失败（Rust 项目适用） | 手写多行调用被 rustfmt 折叠（[`## 3-5 Fork CI 实证要点`](#3-5-fork-ci-实证要点) 第10点） | 本地装 minimal + rustfmt 组件格式化，或靠临时 push-to-main CI 间接确认 |
| clippy -D warnings 报错（Rust 项目适用） | 如 useless_conversion（[`## 3-5 Fork CI 实证要点`](#3-5-fork-ci-实证要点) 第9点） | 在 功能分支(feat) 改，git push origin feat 重验 |
| run 卡 action_required | fork→上游 PR 审批闸门（[`## 3-5 Fork CI 实证要点`](#3-5-fork-ci-实证要点) 第1点） | 等上游维护者 Approve；验证改走 fork 内部 push:[main]（第2点） |
...
#### 3-11-3-1 git 推送 github.com:443 失败的两类情形（务必区分）
...
#### 3-11-3-2 通用可移植方法论（某仓库四门禁级联复盘，脱敏）
...
- CHANGELOG 顶部有对应 `[X.Y.Z] - <date>` 版本段（[`## 3-5 Fork CI 实证要点`](#3-5-fork-ci-实证要点) 第7点），否则 `create-release` 抽 notes 失败。
- `release.yml` 中**发布类 job**（如 `publish-to-crates-io`/`pypi`/`build-python-wheels` 等，取决于该仓库所用语言与发布通道）已加 `if: github.repository == '<upstream>'` 守卫（fork 上求值 false 自动跳过、上游上 true 正常发布），仅保留 `create-release` + `build-release`（[`## 3-5 Fork CI 实证要点`](#3-5-fork-ci-实证要点) 第6点）。⚠️ 禁止写成纯常量 `if: false`（actionlint 拒绝）。
- 未勾 "Require actions pinned to full-length SHA"（[`## 3-5 Fork CI 实证要点`](#3-5-fork-ci-实证要点) 第8点）。
...
### 3-12-2 第二步 — 打标签(tag) 触发（公开动作，暂停等指令）
...
- 需重新触发（同版本重跑）：先删远端标签(tag) 再重推 → `git push origin :refs/tags/v<version>` → `git push origin v<version>`（见 [`## 3-5 Fork CI 实证要点`](#3-5-fork-ci-实证要点) 第3点）。**禁止用 `git push --force-with-lease origin v<version>` 强推标签**，以免与"禁止强推 main"禁令混淆。
...
- **PR open 状态门禁（盲区三·反向保护）**：任何待删分支若仍关联 open PR（无论上游仓库(upstream) 还是 fork 内部 `zhangweildlh/<fork>`），**一律暂停**并明确告知：删除该源分支将使对应 PR 被 GitHub 自动标记为 Closed（即 PR 悬空）。须等你确认「先合并/关闭该 PR」或「明确废弃该分支并连带关闭 PR」后，方可删除。判定依据为 [`### 3-13-1 第一步 — 识别可清理分支`](#3-13-1-第一步--识别可清理分支) 的两条 `--state open` 查询。
- `main` 永不删；当前所在分支不删。
- 优先 `git branch -d`（拒删未合并）；`-D` 强删仅在你明确点名某分支后才用。

### 3-13-4 工区内部对象回收（git gc / reflog / 悬空提交）
...
>
> **单一事实源原则（强制）**：本工作流只定义"PR 全生命周期"的**编排顺序与 PR 专属要求**。凡路径核验、技能激活、本地↔origin↔upstream 对齐、开 PR 命令、文档门禁、CI 轮询、合并、分支清理等**他章已定义的命令/约束/步骤**，一律以「→ 第X章」方式**引用（调用）**，**不在本重复写**。禁止循环引用：本工作流引用他章，他章不反向引用本工作流（仅顶部速查索引表、[`### 3-3-3 Pull Request（《gh pr》）`](#3-3-3-pull-requestgh-pr) 索引作单向导航，不构成逻辑回环）。
>
...
- **born 检查**（防根提交异常，引用 [`### 3-9-2 阶段 1 同步与建分支`](#3-9-2-阶段-1-同步与建分支)）：`git rev-parse HEAD` 须解析出有效 SHA、有父提交；若 `unknown revision` → 先 `git reset --mixed main` 修复，绝不直接 `git add -A && git commit`；
- 分支已推到 你的远端仓库(origin)：`git rev-parse --abbrev-ref HEAD@{upstream}` 应为 `origin/feat/<topic>`；未推则先 `git push -u origin feat/<topic>`（引用 [`### 3-9-3 阶段 2 提交 / 推送 / 触发 CI`](#3-9-3-阶段-2-提交--推送--触发-ci)）；
- 工作区干净（无未提交改动）：`git status --porcelain` 应为空；脏则先提交或 stash，否则不开 PR。

**③ 本地 ↔ origin ↔ upstream 对齐（引用）**：按 [`### 3-8-2 第一步 — 本地 ↔ 你的远端仓库(origin)`](#3-8-2-第一步--本地--你的远端仓库origin)–[`### 3-8-3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)`](#3-8-3-第二步--你的远端仓库originfork--上游仓库upstream) 完成对齐，确保 PR 基于最新 `main`：

- 本地 main 与 origin/main：仅落后/领先→快进拉取/推送（引用 [`### 3-8-2 第一步 — 本地 ↔ 你的远端仓库(origin)`](#3-8-2-第一步--本地--你的远端仓库origin)）；
- origin/main 与 upstream/main：M>0,K>0 冲突→暂停；M=0,K>0→跟随上游（引用 [`### 3-8-3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)`](#3-8-3-第二步--你的远端仓库originfork--上游仓库upstream)）；
...
### 3-14-3 阶段 2 — 重复 PR 检查（引用 + PR 专属口径）

开新 PR 前，先验是否已存在相同/重叠的开放 PR，避免重复开 PR。

- **查询口径（PR 专属，引用 [`### 3-8-3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)`](#3-8-3-第二步--你的远端仓库originfork--上游仓库upstream)）**：一律用**作者口径**，弃用 `--head` 窄口径：

```bash
gh pr list --repo <upstream> --author zhangweildlh --state all
```

- ⚠️ 口径区别（引用 [`### 3-8-3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)`](#3-8-3-第二步--你的远端仓库originfork--上游仓库upstream)）：`--head zhangweildlh:main` 只匹配源分支恰叫 `main` 的 PR，会漏掉 `feat/*`/`fix/*` 分支的 PR；`--author zhangweildlh` 按作者匹配，返回我开的全部 PR，不漏判。仅在查"main→main 通道"时才叠加 `--head`。
- **判定**：
- 已有 open PR 覆盖同一改动 → **不重复开**，直接在那条 PR 上更新（追加提交/编辑正文）；
- 已有 rejected（closed 未合并）PR → 视为 [`### 3-8-4 冲突处理（六类问题）`](#3-8-4-冲突处理六类问题) 问题四，按反馈改后开新 PR（暂停等指令）；
- 无 PR → 进入阶段 3/4 开新 PR。

### 3-14-4 阶段 3 — 查询并遵循上游仓库对 PR 的要求与规范（PR 专属，新内容）

开 PR 前必须先用 `gh` 读取**上游仓库(upstream)** 的 PR 规范与 CI 门槛（引用 [`## 3-7 工作流二 信息读取与搜索`](#3-7-工作流二-信息读取与搜索) gh 优先原则），确保 PR 正文与改动满足上游验收。
...
- 解码：GitHub `contents` API 返 base64，Git Bash 下用 `base64 -d` 解码（引用 [`### 3-7-4 跨命令通用约束`](#3-7-4-跨命令通用约束)）；`gh` 不可用/无权限时回退 WebFetch（引用 [`### 3-7-2 优先顺序（硬规则）`](#3-7-2-优先顺序硬规则) 回退条件）。
...
### 3-14-5 阶段 4 — 开新 PR 的规范操作（引用 + PR 专属补充）

- **开 PR 命令（引用 [`### 3-9-3 阶段 2 提交 / 推送 / 触发 CI`](#3-9-3-阶段-2-提交--推送--触发-ci)）**：
- 向 上游仓库(upstream) 贡献：`gh pr create --repo <upstream> --head zhangweildlh:feat/<topic> --base main`
- fork 内部 PR（触发 fork CI）：`gh pr create --repo zhangweildlh/<fork> --head zhangweildlh:feat/<topic> --base main`
- ⚠️ 多 remote 口径坑（引用 [`### 3-9-3 阶段 2 提交 / 推送 / 触发 CI`](#3-9-3-阶段-2-提交--推送--触发-ci)）：一律显式带 `--repo`，避免默认取 upstream 报 "No commits between…"。
...
**⑤.1 拉取并核对评审意见（引用 [`### 3-3-3 Pull Request（《gh pr》）`](#3-3-3-pull-requestgh-pr) 命令）**：
...
- 分支须已推 origin 且与 PR 关联（引用 [`### 3-14-2 阶段 1 — 开新 PR 前的三核验`](#3-14-2-阶段-1--开新-pr-前的三核验) ②）。
...
**⑤.7 推送更新 PR（引用 [`### 3-9-3 阶段 2 提交 / 推送 / 触发 CI`](#3-9-3-阶段-2-提交--推送--触发-ci) 提交/推送）**：
...
- 自有仓库/自测 PR：`gh pr merge`（引用 [`### 3-3-3 Pull Request（《gh pr》）`](#3-3-3-pull-requestgh-pr)）；
- fork 内部 PR（仅触发 CI）：`gh pr merge --squash`；
- 合并前冲突/分支保护：引用 [`## 3-10 工作流五 多工作树并行开发（–no-ff 普通合并特化）`](#3-10-工作流五-多工作树并行开发no-ff-普通合并特化)（多工作树）或 [`### 3-8-4 冲突处理（六类问题）`](#3-8-4-冲突处理六类问题)（冲突决策树）。

### 3-14-8 阶段 7 — 其他 PR 操作（引用 [`### 3-3-3 Pull Request（《gh pr》）`](#3-3-3-pull-requestgh-pr)，补用法）
...
- 作为评审人审他人 PR：`gh pr review <编号> --approve | --request-changes | --comment`（引用 [`### 3-3-3 Pull Request（《gh pr》）`](#3-3-3-pull-requestgh-pr)）；
- 评论互动：`gh pr comment <编号> --body "..."`（含证据图链接，见 [`### 3-14-6 阶段 5 — PR 审查意见回应（核心，含多轮）（PR 专属，新内容）`](#3-14-6-阶段-5--pr-审查意见回应核心含多轮pr-专属新内容) ⑤.6）。

### 3-14-9 阶段 8 — 收尾与清理（引用）

- 合并后同步：`git switch main && git pull upstream main && git push origin main`；
- 分支清理：引用 [`## 3-13 工作流八 分支清理回收`](#3-13-工作流八-分支清理回收)（已合并分支本地+fork 远程删除、prune 陈旧引用；删前确认 PR 非 open，避免误关 PR，引用 [`### 3-13-3 强门禁（删除专属）`](#3-13-3-强门禁删除专属)）；
- 工区清理：截图 / 构建产物 / 临时测试文件一律**先搜列、后确认、再删除**（不自动删；Git 分支删除不在此项内）。

### 3-14-10 强门禁总述（PR 全生命周期）

- 仅「路径核验通过 + 分支/对齐通过 + 重复 PR 已排除 + 上游规范已遵循 + 正文合规 + CI 全绿」的 PR 可开/可合；
- 一切冲突、公开动作（强推 feat 需 `--force-with-lease` 二次确认、合并受保护分支走 PR、删分支前 PR 状态核验）一律大白话 + 后果 + 暂停等指令（呼应 [`### 3-6-6 激活后执行点索引`](#3-6-6-激活后执行点索引) 强门禁）；
...
### 3-15-2 清理工区工作流（强门禁：先搜列、后确认、再删除）

1. **搜索并详细列出（只读）**：按 [`### 3-15-1 可删除文件定义（清理工区时的目标集）`](#3-15-1-可删除文件定义清理工区时的目标集) 定义，在你指定的工区（默认当前工作目录）搜索全部可删除文件，**逐项列出完整路径 + 大小 + 修改时间 + 类型归属**，生成明细清单呈现给你；此步不删除任何文件。
...
- 明确保留：`CHANGELOG.md`（标准仓库版本记录，**非**"阶段性报告"，不按 [`### 3-15-1 可删除文件定义（清理工区时的目标集）`](#3-15-1-可删除文件定义清理工区时的目标集) 处理）
- **受保护（禁止清理）**：
- `.git/`：版本库本体，删则丢失全部提交历史，永不碰
- `.workbuddy/`：项目记忆库（含 `MEMORY.md` 与 `memory/` 日志），禁止删除
- **边界澄清（避免歧义）**：`tests/`、`scripts/`、`desktop/` 作为仓库**常驻代码目录**整体保留；其中**临时验证用**的测试文件 / 脚本仍按 [`### 3-15-1 可删除文件定义（清理工区时的目标集）`](#3-15-1-可删除文件定义清理工区时的目标集) 判定为可删除（排除冒烟测试），即"目录保留、目录内临时文件可删"。`CHANGELOG.md` 与 [`### 3-15-1 可删除文件定义（清理工区时的目标集）`](#3-15-1-可删除文件定义清理工区时的目标集) 「阶段性报告」互斥，前者永远保留。
