...
> 单一事实源，避免重复——**作用域仅限同一文件内部**（同一文件内任一事实源只在一处权威定义，他处只能引用、不得重述）；**原则上不进行跨文件引用**（Skill 技能作为整体调用不受此限）。**只有先询问用户并获得其显式授权后**，才扩展为同一项目 / 同一工作 / 同一任务中实现和约束；未获授权不得自行做跨文件统一或跨文件单一事源化。
>
> ⚠️ **环境事实边界**：第 2、6 章的工具链路径（如 `D:\Tools\Assembly`）、目录约定、解释器位置均为**本机（Windows 11 / D: 盘）环境事实**，仅对本机通用；**迁移机器须整体更新，切勿当作跨机器通用规则**。账户名、邮箱（第 8 章  `### 8.1 用户名与邮箱`）为跨项目通用身份，不受此限。
>
...
| 会话涉及 mimo.code 协同任务，需要判定是否激活并注入提示词 | `## 第24章 mimo.code 协同（常驻提醒，跨项目）` |
| 要写或执行 .ps1 脚本且内容含中文；PowerShell 脚本中文变乱码 | `## 第25章 PowerShell 脚本中文编码坑（跨项目，2026-07-28 实证，适用于 Windows 环境）` |
| 在 Git Bash 里处理含中文/非 ASCII 的路径或文本；尤其执行删除类命令前 | `## 第26章 Git Bash 下中文/非 ASCII 的字节截断坑（路径误删 · 文本乱码）` |
| 用 cut -c / head -c 等按字节切割工具处理中文，结果变乱码 | `### 26.4 同族坑：文本切割工具把中文截成乱码（《cut -c》 等）` |
...
> 本节为 Node.js / npm / npx 的**通用**单一事实源（Playwright / @playwright/mcp 的浏览器二进制等细节见 `#### 2.4.1 核心原则` 末条）。所有 Node 全局包操作以此处为准。
...
> 用户明确要求：把"每个新对话首条消息先执行 list_groups"加入常驻记忆。

- **触发**：每个新对话（新会话）的**首条用户消息**，必须先执行 `mcp__dynamic-mcp__list_groups`，获取当前通过 dynamic-mcp 已连接的工具组(group)与工具清单，并在首条回复中向用户呈现已连接工具（组名 + 工具名）。
- **调用路径**：`list_groups` 是延迟(deferred)工具，必须经两步：① `ToolSearch` 加载其 schema；② `DeferExecuteTool` 执行 `mcp__dynamic-mcp__list_groups`（无参数）。
- **仅首条执行一次**：同一对话的后续消息不再重复调用；若用户中途新开对话，则新对话首条再次执行。
...
| ------ | ------ | ------ |
| Node.js + npm | D:\Tools\Assembly\nodejs | （见 `### 6.3 严格优先规则`/ `### 2.4 Node.js / npm / npx 全局安装规范` 全局安装铁律） |
...
| gh workflow list / view / run / enable / disable | 管理工作流（fork 启用 Actions 后） | 工作流七 Release 发版（ `## 第18章 工作流六 CI 失败排错`）/ 模块·Fork CI 实证要点（ `## 第12章 Fork CI 实证要点`） |
...
1. **默认使用 GitHub Actions CI 构建**；禁止在本地安装任何编译工具链（MSVC Build Tools、MinGW 等）。若仅用本机已有工具/程序（不安装新工具）即可完成编译，则允许本地编译（详见 `### 2.2 本地编译有条件放开（默认仍走远程 CI）`「本地编译有条件放开」）。
...
- **工作流五 · 多工作树并行开发**（`--no-ff` 普通合并特化）：优先走专属脚本——`sop_worktree_add.sh`（开工作树）/ `sop_worktree_merge.sh`（--no-ff 合并）/ `sop_worktree_cleanup.sh`（回收），均 dry-run 预览、加 `--confirm` 才执行；脚本内置主仓库干净/分支名/工作树路径/一分支一工作树四道守卫 + 冲突预测暂停 + 分支保护 fail-safe 核验。手写命令模板见 `## 第17章 工作流五 多工作树并行开发（–no-ff 普通合并特化）`（仅作原理参考与脚本不可用时的兜底）；文档门禁复用 `sop_docs_sync_check.sh`，分支清理复用工作流八。
- **工作流六 · CI 失败排错**
...
- **工作流七 · Release 发版**（按 `SKILL.md` 流程；公开动作需暂停确认，详见 `## 第18章 工作流六 CI 失败排错` + 第四篇模块「第12章」Fork CI 实证要点）。
- **工作流八 · 分支清理回收**
...
- ⚠️ 选项 A 的 `git reset --hard origin/main` 仅当仓库**不含 `.workbuddy`** 或已备份、且获用户明确授权时方可；含 `.workbuddy` 的仓库按第1.4 禁用 `reset --hard`（防误删项目记忆）。
...
> 注：提交(commit)前文档—代码同步仍须过「第15章 提交前文档同步门禁」（Tier 1 未同步必须先补文档）；本节为合并后补充提交(commit)的专用要求。
...
| code-review-combo（联合代码审查） | 两种互补子技能（委托模式确定性审查 + 五焦点语义深度审查）交叉验证，产出唯一合并审计报告 | 代码审查 / 审计 / 查 BUG 场景。审查与执行两阶段严格分离：审查阶段显式声明"只读、不修改"，修复另起一步；审查脚本一律经 uv run 调用（见 `### 2.3 UV 管理 Python（禁用裸 python / pip）— 原则与详细操作`），不裸跑 python |
...
| mimo-code-collab | GitHub 协同（mimo.code 小米代码智能体）场景 | 详见 `## 第24章 mimo.code 协同（常驻提醒，跨项目）`|
| playwright-360chrome（浏览器自动化） | 真实浏览器导航 / 点击 / 填表 / 截图 / 提取数据 | 需真实浏览器而非静态抓取时；环境约束（锁定本机 360 系内核、库全局安装、禁下载自带浏览器）见 `### 6.2 本地工具安装位置（适用于 Windows 环境）`本地工具安装位置|
...
`mimo-code-collab` 技能由所在平台的技能系统**按名称解析加载**（详见 `### 13.2 激活动作`「绝不写死安装路径」）；不在此处写死任何绝对安装路径（如 `C:/Users/<用户名>/.workbuddy/skills/...`），以免换机/换用户失效。**最新版含 [SKILL.md](http://SKILL.md) + references/：invocation、collab-workflow、test-matrix、cases-and-pitfalls、session-activation-prompt。**
...
## 第25章 PowerShell 脚本中文编码坑（跨项目，2026-07-28 实证，适用于 Windows 环境）

> 关键事实：本机 Windows 11 + PowerShell  `### 5.1 新对话首条消息执行 list_groups（常驻提醒，跨项目）` 默认按**系统 GBK（代码页 936）**读取无 BOM 的 `.ps1` 脚本；而 `Write` 工具生成的 `.ps1` 是 **UTF-8 无 BOM**。两者冲突 → 文件内中文字符串被解析为乱码（如 `涓存椂鏂囦欢`），进而触发 `表达式或语句中包含意外的标记` 类语法解析失败。
...
PowerShell  `### 5.1 新对话首条消息执行 list_groups（常驻提醒，跨项目）` 读 `.ps1` 的默认编码是"系统 ANSI（=GBK）"，仅当文件带 **UTF-8 BOM（EF BB BF）** 时才按 UTF-8 解码。
...
`Set-Content -Encoding UTF8` 在 PowerShell  `### 5.1 新对话首条消息执行 list_groups（常驻提醒，跨项目）` 会写入 **带 BOM** 的 UTF-8；重新运行即恢复正常。
...
## 第26章 Git Bash 下中文/非 ASCII 的字节截断坑（路径误删 · 文本乱码）
...
### 26.4 同族坑：文本切割工具把中文截成乱码（《cut -c》 等）
...
1. 判定"文件是否真的乱码"，**以 `Read` 工具读取结果为准**，不以 Git Bash 管道输出为准；必要时用 `file -bi <文件>` 复核实际编码。
...
> 适用场景：用户要"独立运行的清理/迁移/维护脚本"。与 2.3（UV 管 Python）不冲突——2.3 约束的是我（WorkBuddy）执行 Python 的方式，2.5 约束的是交付给用户独立运行的脚本的形态。

### 2.6 chrome-devtools 技能：主副本 / 部署副本架构与失同步陷阱（2026-08-11 确立）

- **主副本（源/安装源/移植源，必须干净最小）**：`D:\Documents\AI_MCP-Skill-CLI\chrome-devtools`。只含源码 + `localization/` + `skills/` + 示例配置（`*.example.json`）。**严禁落盘**任何可由 Agent 联网下载或运行时衍生的文件：`node_modules`、`build`、`local-config.json`、`mcp-local-config.json`、`.git` 之外的机器专属配置。
- **部署副本（日常使用）**：`C:\Users\15794\.workbuddy\skills\chrome-devtools`（即 `os.homedir()/.workbuddy/skills/chrome-devtools`）。全局 `npm install -g .` 把它做成 `$(npm root -g)/chrome-devtools-mcp` 的**符号链接**，运行直接读该目录；`node_modules`/`build` 也在该目录内。
- **同步机制**：改主副本后，必须从主副本跑 `node mirror_to_target.cjs`（非破坏性覆盖到部署副本，排除 node_modules/build/.git/两个 local-config）+ `node localization/deploy.cjs`（全局装依赖、构建、建符号链接、生成 mcp 配置并合并进 `~/.workbuddy/mcp.json`）。`deploy.cjs` 的 `apply_localize.cjs` 会把 `localization/fragments/*` 注入 `skills/*/SKILL.md`（带 `<!-- LOCALIZED:360Chromex -->` 哨兵，幂等）。
- **铁律**：**任何对该技能的审查/排错，必须以主副本为唯一权威基准**，绝不可只盯部署副本——部署副本若未重跑 mirror+deploy 就会与主副本失同步，制造"孤儿引用"等**假象 BUG**（例：主副本已有 `skills/chrome-devtools-cli/SKILL.md`，但陈旧部署副本缺它，导致 `apply_localize.cjs` 对其 `inject` 看似"静默跳过"）。真 BUG 只认主副本源码。
- **浏览器启动归属**：`verify_browser.cjs` + `start.cjs`（按 `local-config.json` 以 `--user-data-dir` 复用登录态、非交互、detached 后台拉起 360Chromex）**由 Agent / Skill 工作流自主执行，绝不要求用户手动敲**。SKILL.md 步骤 1 已强制此约定。
- **MCP 连接前置**：MCP 服务器自身（stdio）与浏览器是否在 9222 监听**解耦**——握手不依赖浏览器；但工具调用需浏览器在线。故"连接器 disconnected"或"9222 无监听"属**运行时/程序化前置**，非技能代码缺陷，靠 Agent 跑 `start.cjs` 拉起浏览器 + 在连接器管理页"信任"解决，不靠改主副本代码。
