...
scope_in: "代码修改与 BUG 修复的优先级裁决（契约保真 > 正确性 > 覆盖完整 > 最小作用域 > 可验证）；最小作用域；改完即验证；改完后的迭代审查与质量门禁闭环（静态审计先于动态测试、P0/P1/P2 分级、5轮上限）；路径核验四规则；任务记忆须记录操作目录"
...
| 适用场景 | 章节位置 | 备注 |
|---------|---------|------|
| 2-1 代码编写、修改和BUG修复通用纪律 | `## 2-1 代码编写、修改和BUG修复通用纪律` |  |
| 2-2 路径核验 | `## 2-2 路径核验` |  |
| 2-2-1 规则一：先核验"要操作的目录"再动手 | `### 2-2-1 规则一：先核验"要操作的目录"再动手` |  |
| 2-2-2 规则二：路径核验的正确命令写法 | `### 2-2-2 规则二：路径核验的正确命令写法` |  |
| 2-2-3 规则三：任务记忆/工作日志必须记录"操作目录" | `### 2-2-3 规则三：任务记忆/工作日志必须记录"操作目录"` |  |
| 2-2-4 规则四：发现异常先问，对齐后再做 | `### 2-2-4 规则四：发现异常先问，对齐后再做` |  |
| 2-3 修改代码后的审查和BUG修复迭代闭环（质量门禁） | `## 2-3 修改代码后的审查和BUG修复迭代闭环（质量门禁）` |  |
| 2-3-1 单轮标准步骤（按序执行，不得跳步） | `### 2-3-1 单轮标准步骤（按序执行，不得跳步）` |  |
| 2-3-2 新一轮触发与终止 | `### 2-3-2 新一轮触发与终止` |  |
| 2-3-3 授权边界与红线 | `### 2-3-3 授权边界与红线` |  |
<!-- INDEX_END -->
## 2-1 代码编写、修改和BUG修复通用纪律
...
### 2-2-4 规则四：发现异常先问，对齐后再做

- 一旦发现路径异常（如 `git rev-parse --show-toplevel` 指向的不是预期仓库目录、根目录意外出现 `.git`、工作区文件出现在错误层级），**立即暂停，先问用户**，大白话说明发现了什么。
- **先排除"路径格式误报"再判定异常**：`git rev-parse` 报 `not a git repository` 时，**第一动作必须是 `ls "<目录>/.git"` 复核**；确认 `.git` 不存在才视为真异常并触发暂停。若 `.git` 存在，则是命令路径格式问题（见 [`### 2-2-2 规则二：路径核验的正确命令写法`](#2-2-2-规则二路径核验的正确命令写法) 规则二），改用正确写法重测，**不得据此触发"路径异常暂停"**，也不向用户发"路径核验关键异常"类告警。
- **只有与用户明确一致"要操作的文件夹路径"后，才能继续讨论工作流程、操作步骤和下一步内容。** 绝不绕过路径对齐直接进入执行。

---

## 2-3 修改代码后的审查和BUG修复迭代闭环（质量门禁）

> 本纪律是 [`## 2-1 代码编写、修改和BUG修复通用纪律`](#2-1-代码编写修改和bug修复通用纪律) 的"事后端到端质量门禁"深化——2-1 规定"怎么改"（六条铁律 / 统一优先级裁决器 / 全局契约面 / 单轮回归纪律），本纪律规定"改完之后怎么循环审、怎么判无阻塞 BUG、到哪停"。两者闭环，本纪律不重述 2-1 内容，仅引用。
>
...
- 若仍存在 P0/P1/P2 未关闭、必需测试失败、新增缺陷、修复未验证或 flaky → 启动新一轮迭代（回到 [`### 2-3-1 单轮标准步骤（按序执行，不得跳步）`](#2-3-1-单轮标准步骤按序执行不得跳步)）。
- 全部阻塞项解决（满足质量门禁）才终止。
