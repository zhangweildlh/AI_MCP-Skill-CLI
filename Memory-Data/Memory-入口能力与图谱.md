...
role: "能力台账层"
theme: "dynamic-mcp 统一入口 / 常用技能 / DeusData 代码图谱 / memory-mgr.py 的单工具调用说明"
scope_in: "dynamic-mcp 调用三步与分组清单；常用跨项目技能（单技能调用说明）；DeusData 本地代码知识图谱的接入与单工具用法；memory-mgr.py 的位置 / 用途 / 常用命令 / 注意事项"
scope_out: "多个工具/多技能协同的作业流程（如搜索 SOP）→ 子文件6；能力使用中的踩坑 → 子文件4；环境禁令 → 子文件1；GitHub 流程 → 子文件3"
summary: "Dynamic-mcp统一能力入口、常用跨项目技能、DeusData本地代码知识图谱（mimo-mcp 当前已停用）"
...
| 5-2 常用跨项目技能（常驻工具链，跨项目） | `## 5-2 常用跨项目技能（常驻工具链，跨项目）` |  |
| 5-3 DeusData本地代码知识图谱（经 dynamic... | `## 5-3 DeusData本地代码知识图谱（经 dynamic-mcp 接入）` |  |
| 5-4 memory-mgr.py 维护工具（日常维护入口） | `## 5-4 memory-mgr.py 维护工具（日常维护入口）` |  |
| 5-4-1 工具位置 | `### 5-4-1 工具位置` |  |
| 5-4-2 核心用途 | `### 5-4-2 核心用途` |  |
| 5-4-3 常用命令 | `### 5-4-3 常用命令` |  |
| 5-4-4 注意事项 | `### 5-4-4 注意事项` |  |
<!-- INDEX_END -->
...
| zvec-grep | 向量检索 | 语义代码/文档检索 |
| TickTick | 待办事项 | 任务管理 |
| Everything-search | 本地文件名/路径全文检索 | 快速定位本地文件 |

> 注：`mimo-mcp`（mimo.code 协同）与 `playwright-360chrome`（浏览器自动化）当前在 `dynamic-mcp.json` 中标为 `"enabled": false`，未出现在连接分组中；如需启用须先修改配置并重启代理。

---

## 5-2 常用跨项目技能（常驻工具链，跨项目）

> 经多会话实证、跨项目复用的常驻技能；与 git/gh 流程互补（路径核验另见 [## 7 路径核验](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-代码纪律与Git操作.md#2-2-路径核验)），涉及对应场景时主动加载。
>
...
- **code-review-combo（联合代码审查）**：两种互补子技能（委托模式确定性审查 + 五焦点语义深度审查）交叉验证，产出唯一合并审计报告 — 代码审查 / 审计 / 查 BUG 场景。审查与执行两阶段严格分离：审查阶段显式声明"只读、不修改"，修复另起一步；审查脚本一律经 [### 3.3 UV 管理 Python（禁用裸 python / pip）— 原则与详细操作](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-全局禁令与环境约束.md#1-3-3-uv-管理-python禁用裸-python--pip-原则与详细操作) uv run 调用，不裸跑 python
- **github-personal-manager（GitHub 管理统一执行）**：提交 / 推送 / 开 PR / 同步 / 发版 / 分支清理等本地与远端 GitHub 操作的一站式技能 — 凡涉及 GitHub 仓库/项目/代码操作，必须先按名称激活此技能（详见 [## 13 工作流一 github-personal-manager 自动激活（GitHub 工作流总闸门，前置）](file:///D:/Documents/AI_MCP-Skill-CLI/Memory-Data/Memory-GitHub全流程操作.md#3-6-工作流一-github-personal-manager-自动激活github-工作流总闸门前置)）
...
## 5-3 DeusData本地代码知识图谱（经 dynamic-mcp 接入）
...
- **关联**：详细工具表与 Cypher 查询范式见 Skill `codebase-memory`。
