...
scope_in: "PowerShell 脚本中文编码坑；Git Bash 中文截断与切割工具乱码；Node/JS 解析本地文件须兼容 CRLF；向用户 JSON 配置合并须字段级；多 Agent 长任务协作闭环；Python 脚本删除护栏"
...
| 适用场景 | 章节位置 | 备注 |
|---------|---------|------|
| 4-1 PowerShell脚本中文编码坑 | `## 4-1 PowerShell脚本中文编码坑` |  |
| 4-1-1 表现 | `### 4-1-1 表现` |  |
| 4-1-2 根因 | `### 4-1-2 根因` |  |
| 4-1-3 修复（两步） | `### 4-1-3 修复（两步）` |  |
| 4-1-4 预防 | `### 4-1-4 预防` |  |
| 4-1-5 注意 | `### 4-1-5 注意` |  |
| 4-2 Git Bash下中文截断坑 | `## 4-2 Git Bash下中文截断坑` |  |
| 4-2-1 现象与根因 | `### 4-2-1 现象与根因` |  |
| 4-2-2 铁律（任何项目、任何目录都适用） | `### 4-2-2 铁律（任何项目、任何目录都适用）` |  |
| 4-2-3 误删后的恢复思路 | `### 4-2-3 误删后的恢复思路` |  |
| 4-2-4 同族坑：文本切割工具把中文截成乱码（《cut -... | `### 4-2-4 同族坑：文本切割工具把中文截成乱码（《cut -c》 等）` |  |
| 4-3 工程实现跨项目避坑铁律 | `## 4-3 工程实现跨项目避坑铁律` |  |
| 4-3-1 Node/JS 解析本地文件必须兼容 CRLF ... | `### 4-3-1 Node/JS 解析本地文件必须兼容 CRLF 行尾` |  |
| 4-3-2 向用户 JSON 配置合并条目必须字段级合并 | `### 4-3-2 向用户 JSON 配置合并条目必须字段级合并` |  |
| 4-3-3 多 Agent 并行长任务协作闭环（审查-修复-... | `### 4-3-3 多 Agent 并行长任务协作闭环（审查-修复-复审计 + 503 续派）` |  |
| 4-3-4 删除护栏补充雷区（Python 脚本） | `### 4-3-4 删除护栏补充雷区（Python 脚本）` |  |
<!-- INDEX_END -->
## 4-1 PowerShell脚本中文编码坑
...
2. 脚本内字符串应使用英文/ASCII；仅在"写文件"环节使用 UTF-8 输出；
...
> 跨项目环境禁忌。**同一根因**：Git Bash（MinGW/MSYS2）对多字节字符按**字节**而非**字符**处理，把中文从中间腰斩。两种表现——[`23.1`](#4-1-1-表现)~[`23.3`](#4-1-3-修复两步) 讲**路径被截断导致误删**（后果不可逆），[`23.4`](#4-1-4-预防) 讲**文本被截断导致误读**（后果是误判）。务必永久遵守。
...
### 4-2-4 同族坑：文本切割工具把中文截成乱码（《cut -c》 等）
...
### 4-3-1 Node/JS 解析本地文件必须兼容 CRLF 行尾
...
### 4-3-3 多 Agent 并行长任务协作闭环（审查-修复-复审计 + 503 续派）
