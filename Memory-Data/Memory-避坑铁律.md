...
updated: "2026-09-18T15:35:00+08:00"
parent: "MEMORY.md"
...
| 4-4 记忆维护工具链（memory-mgr.py）避坑铁律 | `## 4-4 记忆维护工具链（memory-mgr.py）避坑铁律` |  |
| 4-4-1 插入新章后慎用 `rewrite`，其编号碰撞处... | `### 4-4-1 插入新章后慎用 `rewrite`，其编号碰撞处理会丢章` |  |
| 4-4-2 工具链跑完后必须逐章 grep 核验正文完整性 | `### 4-4-2 工具链跑完后必须逐章 grep 核验正文完整性` |  |
| 4-4-3 手写锚点先 grep 文件既有同级锚点字符串，避... | `### 4-4-3 手写锚点先 grep 文件既有同级锚点字符串，避免偏差` |  |
| 4-4-4 YAML 字段编辑前必须 re-read 真实行... | `### 4-4-4 YAML 字段编辑前必须 re-read 真实行，凭记忆拼串必失败` |  |
<!-- INDEX_END -->
...
## 4-4 记忆维护工具链（memory-mgr.py）避坑铁律

> 适用场景（强制绑定）：使用本仓库 `Memory-Data/memory-mgr.py`（`index` / `rewrite` / `check`）与 `断链检测.py` 维护任一子记忆文件、向其中插入新章（尤其「已存在 N-M 含子节 + 新插入 N-(M+1) 含子节」结构）时。四条均为 2026-09-18 在维护 `Memory-代码纪律与Git操作.md` 时实证踩坑并恢复，作永久避坑。

### 4-4-1 插入新章后慎用 `rewrite`，其编号碰撞处理会丢章

- **现象**：在 `Memory-代码纪律与Git操作.md` 已存在 `## 2-4`(含 2-4-1/2/3) 且新插入 `## 2-5`(含 2-5-1/2/3) 后运行 `rewrite`，整章 `## 2-4 开发态/部署态分离` 的内容从正文消失；`check` 12 维度全过却未报警，`断链检测.py` 报 3 处断链才暴露内容缺漏。
- **根因**：`rewrite` 编号重建在「同级已存在章含子节 + 新插入章含子节」结构下发生编号碰撞，误将新章重编号覆盖旧章并丢弃旧章正文（非编辑失误，是工具缺陷）。
- **修复（恢复）**：从本轮备份整体还原（`cp` 备份覆盖），重做三处编辑，`rewrite` 改为跳过。
- **预防（铁律）**：插入新章后，凡结构为「已存在 N-M(含子节) + 新 N-(M+1)(含子节)」，**禁止运行 `rewrite`**；手动正确编号，仅靠 `index`（仅重建索引区，不改正文）+ `check` + `断链检测.py` 验收。

### 4-4-2 工具链跑完后必须逐章 grep 核验正文完整性

- **现象**：`check` 全绿，但 `rewrite` 已丢整章；仅凭 `check` 绿无法发现内容缺漏，直到 `断链检测.py` 报断链才察觉。
- **根因**：`check` 查格式/编号/锚点格式，不查内容是否缺失；`断链检测.py` 查链接可解析性，能间接暴露（丢章→旧章锚点被新章引用→断链），但必须主动跑、主动看。
- **预防（铁律）**：任何工具链（尤其 `rewrite`）跑完后，**必跑 `断链检测.py`**，并 **`grep "^## N-"` 逐章核验正文标题序列完整、无缺章、无编号碰撞**，两者皆绿再交付；`check` 绿 ≠ 内容完整。

### 4-4-3 手写锚点先 grep 文件既有同级锚点字符串，避免偏差

- **现象**：写 `## 2-4` 引用链接时手写锚点 `#2--1-...`（多一横）导致断链；`## 2-4` 章节锚点因标题含斜杠 `/`、全角括号，与人工推导 slug 不一致，`断链检测.py` 报 3 处断链。
- **根因**：Markdown 锚点生成规则对中文标题、特殊字符（全角括号、斜杠、空格）处理与人工推导不同；手工拼锚点必错。
- **预防（铁律）**：回填任何 `(#...)` 链接前，先 `grep` 文件既有同级标题的锚点字符串（或先跑 `index` 再从索引区复制权威锚点），用真实锚点回填；不凭记忆手写。新建章内若无法确认锚点，优先用纯文本提及章节号（如「见 `## 4-3`」），不建链接。

### 4-4-4 YAML 字段编辑前必须 re-read 真实行，凭记忆拼串必失败

- **现象**：多次 `Edit` 的 `scope_in` 因 old_string 与磁盘真实字节细微差异（如该行已含 2-4 收录描述而我误判为简版）比对失败、未生效，需 re-read 后才修正。
- **根因**：记忆文件经多轮工具改写，YAML 行内容已变；凭「上轮印象」拼 old_string 与实际不符。
- **预防（铁律）**：每次编辑 YAML（`updated` / `scope_in` 等）前，先 `Read` 该行（或 `Grep` 抓行）确认真实文本，用真实文本作 old_string；禁止凭记忆拼串。
