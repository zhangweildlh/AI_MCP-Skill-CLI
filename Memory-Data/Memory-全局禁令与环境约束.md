...
| 适用场景 | 章节位置 | 备注 |
|---------|---------|------|
| 1-1 总则 · 信任但必须验证（跨 SOP 全局） | `## 1-1 总则 · 信任但必须验证（跨 SOP 全局）` |  |
| 1-2 全局禁令（最高优先级） | `## 1-2 全局禁令（最高优先级）` |  |
| 1-2-1 禁止强推/删除自家 main（及受保护分支） | `### 1-2-1 禁止强推/删除自家 main（及受保护分支）` |  |
| 1-2-2 三段式二次授权铁律 | `### 1-2-2 三段式二次授权铁律` |  |
| 1-2-3 入库隐私闸门（2026-08-04 实测拦截后确... | `### 1-2-3 入库隐私闸门（2026-08-04 实测拦截后确立）` |  |
| 1-2-4 Git 撤销/回退安全（reset 三模式 + ... | `### 1-2-4 Git 撤销/回退安全（reset 三模式 + 硬约束）` |  |
| 1-3 环境约束 | `## 1-3 环境约束` |  |
| 1-3-1 Docker 禁用 | `### 1-3-1 Docker 禁用` |  |
| 1-3-2 本地编译有条件放开（默认仍走远程 CI） | `### 1-3-2 本地编译有条件放开（默认仍走远程 CI）` |  |
| 1-3-3 UV 管理 Python（禁用裸 python ... | `### 1-3-3 UV 管理 Python（禁用裸 python / pip）— 原则与详细操作` |  |
| 1-3-4 Node.js / npm / npx 全局安装... | `### 1-3-4 Node.js / npm / npx 全局安装规范` |  |
| 1-3-5 交付脚本须纯标准库、禁用 WorkBuddy 自... | `### 1-3-5 交付脚本须纯标准库、禁用 WorkBuddy 自带运行时（2026-08-11 确立）` |  |
| 1-4 会话启动闸门 | `## 1-4 会话启动闸门` |  |
| 1-4-1 新对话首条消息执行 list_groups（常驻... | `### 1-4-1 新对话首条消息执行 list_groups（常驻提醒，跨项目）` |  |
...
### 1-2-3 入库隐私闸门（2026-08-04 实测拦截后确立）
...
### 1-2-4 Git 撤销/回退安全（reset 三模式 + 硬约束）
...
### 1-3-3 UV 管理 Python（禁用裸 python / pip）— 原则与详细操作
...
#### 1-3-3-4 运行 Python 程序（必须用 uv run）
...
#### 1-3-3-6 安装前「2 项检查」（全部未命中才允许 uv add）
...
### 1-3-5 交付脚本须纯标准库、禁用 WorkBuddy 自带运行时（2026-08-11 确立）
...
2. **纯标准库优先**：交付脚本应优先使用 Python 标准库；仅在标准库无法满足需求时，才考虑使用第三方包。（sqlite3/os/shutil/json/argparse 等），保证用户任意 Python 3.8+ 直接 `python xxx.py` 可跑，无需 uv/虚拟环境/第三方包。
...
### 1-4-1 新对话首条消息执行 list_groups（常驻提醒，跨项目）
