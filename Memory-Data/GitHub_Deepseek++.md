...
> 跨项目永久记忆提炼版，本文件聚焦 **GitHub 操作**，并保留支撑性通用基础约束（环境约束与 Docker 禁用、回答风格、目录工具链、代码编写修改通用纪律、账号身份、gh 能力边界）；原 MEMORY.md 中与会话启动闸门（Dynamic-mcp / list_groups）、PowerShell/Git Bash 编码坑、工程避坑铁律、DeusData 图谱等非 GitHub 直接相关章节未纳入。  
> 本文件主体自 `C:\Users\15794\.workbuddy\MEMORY.md`（2026-08-26 整理版）同步修订，GitHub 篇严格按仓库从创建到收尾的全生命周期顺序排列。  
> 按「**全局基础约束 → 本地环境与工具链 → 代码纪律 → GitHub 全生命周期标准工作流 → 技能与常驻提醒**」五篇组织。  
> 单一事实源，避免重复；不同章节对同一事源/事项/要求/流程/操作的描述必须一致、互不矛盾。
>
> ⚠️ **环境事实边界**：第 2 章、第 4 章的工具链路径（如 `D:\Tools\Assembly`）、目录约定、解释器位置均为**本机（Windows 11 / D: 盘）环境事实**，仅对本机通用；**迁移机器须整体更新，切勿当作跨机器通用规则**。账户名、邮箱（第 7 章 `### 7.1 用户名与邮箱`）为跨项目通用身份，不受此限。
>
> ⚠️ **项目特定内容边界**：本跨项目记忆只沉淀**通用方法、硬禁令、环境约束与身份**。任何仅对某个具体仓库成立的操作约束、当前状态、个性化协作规则，一律归属该仓库的**项目级记忆**（`D:\Documents\AI_Work_Temp\<仓库>\.workbuddy\memory/MEMORY.md`），不在本文件重复（详见第 7 章 `### 7.2 Fork 仓库清单（账户资产索引）` 声明）。
...
| 需要确定工作目录 / Skill 目录 / GitHub 仓库根目录 / 临时目录 / 工具根目录 到底在哪                                              | `### 4.1 目录根约定`                                               |
| 要调用 Node / UV / Python / Git / gh / ffmpeg / WMIC 等本机工具，需要它们的安装绝对路径                                 | `### 4.2 本地工具安装位置（适用于 Windows 环境）`                            |
| 同一个工具本机有多个版本（内置运行时 vs 自装工具链），需要决定用哪个                                                                | `### 4.3 严格优先规则`                                              |
| 任何「代码编写 / 代码修改 / BUG 修复 / PR 评审 / PR 全生命周期」任务都必须遵循：完整裁决器 + 全局契约面 + 全仓库整体性视角（变更波及半径 + 回归纪律 + 既有功能保真） | `## 第5章 代码编写、修改和BUG修复通用纪律`                                    |
| 担心改完把原来好用的功能弄坏；需要证明零回归；想跳过 / 删除测试或下调覆盖率·lint·超时阈值来凑绿；想删掉「看起来没用」的既有代码（对应红线⑦ + 「既有功能保真专项」）            | `## 第5章 代码编写、修改和BUG修复通用纪律`                                    |
| 要执行 git 或 gh 命令，需要可执行文件的绝对路径                                                                        | `### 6.1 git / gh 本地路径`                                       |
| git 推送 / gh 调用出现认证失败、令牌失效、要求输入密码                                                                    | `### 6.2 凭证助手桥接`                                              |
| 判断某件事 gh 到底能不能做（能力边界），避免设计出跑不通的方案                                                                   | `### 6.4 能力边界（重要）`                                            |
| 需要提交作者信息、配置 user.name / user.email、确认账号身份                                                           | `### 7.1 用户名与邮箱`                                              |
| 需要知道我名下有哪些 fork 仓库、各自的上游是谁、本地路径在哪                                                                   | `### 7.2 Fork 仓库清单（账户资产索引）`                                   |
| 想知道 gh 命令行整体能做什么；要挑一条合适的 gh 子命令                                                                     | `## 第8章 gh 能力速查`                                              |
| 要创建 / 查看 / 评论 / 合并 / 关闭 PR，需要具体的 gh pr 命令写法                                                         | `### 8.3 Pull Request（《gh pr》）`                               |
| 要在 GitHub 全站搜代码、搜仓库、搜 issue                                                                         | `### 8.5 全站搜索（《gh search》）`                                   |
| gh 子命令覆盖不到的功能，需要直接打 GitHub REST / GraphQL 接口                                                        | `### 8.6 原生 API 访问（《gh api》）`                                 |
| 要查看 CI 运行状态、拉失败日志、重跑失败 job、手动触发 workflow                                                            | `### 8.7 CI/CD（《gh run》 / 《gh workflow》）`                     |
| 要创建 Release、上传或下载发布产物                                                                               | `### 8.8 发布与制品（《gh release》）`                                 |
| 每个新任务、每条 git/gh 流程开工前的第 0 步；不确定当前在哪个目录                                                              | `## 第9章 路径核验`                                                 |
| 要写出正确的路径核验命令（含中文路径、含空格路径的处理）                                                                        | `### 9.2 规则二：路径核验的正确命令写法`                                     |
| 路径核验结果与预期不符、找不到 .git、目录疑似不对                                                                         | `### 9.4 规则四：发现异常先问，对齐后再做`                                    |
| 会话中出现任何 GitHub 相关意图（仓库、分支、PR、CI、发版），动手前的总闸门                                                         | `## 第10章 工作流一 github-personal-manager 自动激活（GitHub 工作流总闸门，前置）` |
| 技能已激活，需要按意图跳到对应的工作流章节或运维脚本                                                                          | `### 10.6 激活后执行点索引`                                           |
| 用户给了一个 GitHub 网址要我读；要搜某个仓库、某段代码、某个 issue                                                            | `## 第11章 工作流二 信息读取与搜索`                                        |
| 读取 GitHub 信息时该用 gh、WebFetch 还是网页抓取，需要决定优先级                                                          | `### 11.2 优先顺序（硬规则）`                                          |
| 例行仓库巡检：本地 ↔ 你的远端仓库(origin) ↔ 上游仓库(upstream) 三方同步状态检查                                                | `## 第12章 工作流三 日常同步巡检`                                         |
| 同步时出现冲突、分叉、落后、领先、脏工作区等异常，需要判断后果并决定是否暂停                                                              | `### 12.4 冲突处理（六类问题）`                                         |
| 本地目录没有 .git、仓库文件残缺、本地存在远端没有的独有文件                                                                    | `### 12.5 特殊场景（仓库残缺/本地无 .git / 本地独有文件）`                       |
| 要改代码、提交、推送、开 PR、合并的标准全流程（单分支常规改动）                                                                   | `## 第13章 工作流四 标准代码修改`                                         |
| 改完代码要提交推送并触发 CI；需要 commit 规范、push 写法、CI 触发条件、PR 正文 --body-file 用法                                   | `### 13.3 阶段 2 提交 / 推送 / 触发 CI`                               |
| 改到一半被打断、要临时切分支、需要暂存未完成的改动                                                                           | `### 13.8 实用工具：stash 临时抽屉（被打断时保存进度）`                          |
| 多个任务要并行开发、互不干扰；需要开多个工作树(worktree)并最终普通合并回主线                                                         | `## 第14章 工作流五 多工作树并行开发（–no-ff 普通合并特化）`                        |
| 功能分支开发完，要以保留分支拓扑的方式合并回 main                                                                         | `### 14.8 阶段三：《--no-ff》 合并到主线(main)`                          |
| 并行开发全流程走完，需要逐项验收打勾确认没有遗漏                                                                            | `### 14.13 验收闸门清单（四张，逐项打勾）`                                   |
| 每次 git commit 之前，判断本次改动是否需要同步 README / CHANGELOG / 文档                                               | `## 第15章 提交前文档同步门禁`                                           |
| 需要执行文档同步检查的具体步骤（查询 → 分析 → 需改先改 → 提交 → 推送）                                                           | `### 15.3 执行流程（查询 → 分析 → 需改先改 → 提交 → 推送）`                     |
| fork 仓库的 CI 行为异常：发布类 job 误触发、rustfmt/clippy 报错、分支保护、审批闸门                                            | `## 第16章 Fork CI 实证要点`                                        |
| CI 变红了，需要定位失败原因并修复回推                                                                                | `## 第17章 工作流六 CI 失败排错`                                        |
| git push 报 github.com:443 连接超时或被重置，需要区分偶发瞬断与网络封锁并选择绕行方案                                             | `#### 17.3.1 git 推送 github.com:443 失败的两类情形（务必区分）`             |
| CI 多道门禁串行连环失败（lint / 类型 / i18n / 产物校验），需要逐轮排查方法论                                                    | `#### 17.3.2 通用可移植方法论（某仓库四门禁级联复盘，脱敏）`                         |
| PR 相关的任何操作：开新 PR、查重、遵循上游规范、回应评审、合并、关闭、收尾                                                            | `## 第18章 工作流九 PR 全生命周期操作`                                     |
| 向某个上游仓库提 PR 前，需要查清它的 CONTRIBUTING、PR 模板、必过 CI 检查                                                    | `### 18.4 阶段 3 — 查询并遵循上游仓库对 PR 的要求与规范（PR 专属，新内容）`             |
| PR 流程各阶段的强制门禁汇总，动手前逐项确认                                                                             | `### 18.10 强门禁总述（PR 全生命周期）`                                   |
...
| 要清理工区里的垃圾文件、过期产物、临时脚本、截图、报告                                                                         | `## 第21章 工作流十 清理工区维护`                                         |
| 判断某个文件属不属于"可删除"（阶段性报告、开发日志、审查报告、临时测试、构建产物）                                                          | `### 21.1 可删除文件定义（清理工区时的目标集）`                                 |
| 判断某个文件是否受保护、绝对不能删                                                                                   | `### 21.3 必须保留与受保护清单（清理工区永不删）`                                |
| 需要挑选一个跨项目常用技能（深度讨论 / 代码审查 / GitHub 管理 / 联网搜索 / 记忆整合等）                                               | `## 第22章 常用跨项目技能（常驻工具链，跨项目）`                                  |

---

# 第一篇：全局基础约束

> 本篇为最高优先级规则与通用基础约束，所有操作前必须遵守。
...
> 适用场景：用户要"独立运行的清理/迁移/维护脚本"。与 `### 2.3 UV 管理 Python（禁用裸 python / pip）— 原则与详细操作` 不冲突——该节约束的是我（WorkBuddy）执行 Python 的方式，本节约束的是交付给用户独立运行的脚本的形态。
...
# 第三篇：代码编写、修改和BUG修复通用纪律

> 本篇收纳被多个工作流引用的通用工程纪律。

## 第5章 代码编写、修改和BUG修复通用纪律
...
# 第四篇：GitHub 全生命周期标准工作流

> 本篇按仓库从创建到收尾的全生命周期顺序编排。**⚠️ 路径核验（`## 第9章 路径核验`）是最高优先级硬规则：每个工作流启动阶段的第一步都必须是路径核验，未核验路径不得进入任何后续步骤。**
...
| we-mp-rss        | <https://github.com/zhangweildlh/we-mp-rss>        | rachelos/we-mp-rss          | 私人化定制 Fork（专属协作规则见其项目级记忆，不在此重复） |
...
### 8.10 密钥与变量（《gh secret》 / 《gh variable》）
...
### 9.4 规则四：发现异常先问，对齐后再做

- 一旦发现路径异常（如 `git rev-parse --show-toplevel` 指向的不是预期仓库目录、根目录意外出现 `.git`、工作区文件出现在错误层级），**立即暂停，先问用户**，大白话说明发现了什么。
- **先排除"路径格式误报"再判定异常**：`git rev-parse` 报 `not a git repository` 时，**第一动作必须是 `ls "<目录>/.git"` 复核**；确认 `.git` 不存在才视为真异常并触发暂停。若 `.git` 存在，则是命令路径格式问题（见 `### 9.2 规则二：路径核验的正确命令写法` 规则二），改用正确写法重测，**不得据此触发"路径异常暂停"**，也不向用户发"路径核验关键异常"类告警。
- **只有与用户明确一致"要操作的文件夹路径"后，才能继续讨论工作流程、操作步骤和下一步内容。** 绝不绕过路径对齐直接进入执行。

---

## 第10章 工作流一 github-personal-manager 自动激活（GitHub 工作流总闸门，前置）

> 用户明确要求：把"`github-personal-manager` 技能自动激活"加入常驻记忆。凡涉及 GitHub 仓库/项目/代码操作的任务，主 Agent 必须在动手前先判断是否已激活该技能。
>
> 本规则作为 GitHub 工作流总闸门，逻辑上前置于本篇各工作流章节。本篇各工作流在启动前均须先激活本技能。
...
该技能是 GitHub 个人操作的统一执行入口，覆盖「信息读取与搜索 / 日常同步巡检 / 标准代码修改 / 多工作树并行开发 / CI 失败排错 / Release 发版 / 分支清理回收 / PR 全生命周期 / 清理工区」等本篇全部 GitHub 工作流（其中自动激活为本总闸门自身）；凡上述场景优先走该技能，而非手工拼 git/gh 命令。**各工作流章节已统一标注"启动前必须先完成路径核验"，即本门禁的落地**。

### 10.4 与现有 GitHub 规则的关系
...
- `scripts/sop_docs_sync_check.sh <仓库路径> [--strict]`（只读·提交前文档同步门禁，对应 `## 第15章 提交前文档同步门禁`）
- `scripts/sop_pr_create.sh <仓库路径> --base <分支> [--confirm]`（开 PR）
- `scripts/sop_pr_checks.sh <仓库路径>`（只读·轮询 CI）
- **工作流五 · 多工作树并行开发**（`--no-ff` 普通合并特化）：优先走专属脚本——`sop_worktree_add.sh`（开工作树）/ `sop_worktree_merge.sh`（--no-ff 合并）/ `sop_worktree_cleanup.sh`（回收），均 dry-run 预览、加 `--confirm` 才执行；脚本内置主仓库干净/分支名/工作树路径/一分支一工作树四道守卫 + 冲突预测暂停 + 分支保护 fail-safe 核验。手写命令模板见工作流五章节（仅作原理参考与脚本不可用时的兜底）；文档门禁复用 `sop_docs_sync_check.sh`，分支清理复用 `## 第20章 工作流八 分支清理回收`。
- **工作流六 · CI 失败排错**
...
- **工作流七 · Release 发版**（按 `SKILL.md` 流程；公开动作需暂停确认，详见 `## 第17章 工作流六 CI 失败排错` + `## 第16章 Fork CI 实证要点`）。
- **工作流八 · 分支清理回收**
- `scripts/sop_branch_merged_status.sh <仓库路径>`（只读·合并状态 + open PR 反向识别）
- `scripts/sop_fetch_prune.sh <仓库路径> [--confirm]`（清理本地过时远程跟踪引用）
- **工作流九 · PR 全生命周期操作**（开新PR/重复核查/上游规范遵循/响应评审/合并/关闭/其他 PR 操作）：开 PR / 回复 PR 用 `scripts/sop_pr_create.sh <仓库路径> --base <分支> [--confirm]`（引用 工作流四 提交推送触发 CI 阶段）；CI 轮询 `scripts/sop_pr_checks.sh <仓库路径>`；其余 PR 交互（view/diff/comment/edit/close/review）按 Pull Request 命令速查；评审回应与截图上传的完整编排见 `## 第18章 工作流九 PR 全生命周期操作`。
...
- **参考资料（references/）**：
- `references/docs-sync-checklist.md`：提交前文档同步「分层检查清单」权威定义（Tier 1/2/3 + 改动类型映射），对应 `## 第15章 提交前文档同步门禁`。
- `references/fork-ci-pitfalls.md`：Fork CI 实证要点与编译构建规则（标签重推、发布 job 守卫、clippy/rustfmt 坑、443 两类情形与 REST 绕过），对应 `## 第16章 Fork CI 实证要点`。
- `references/gh-capability.md`：`gh` CLI 能力全览与能力边界（哪些 `gh` 不能做、归 `git`/CI），对应 `## 第8章 gh 能力速查`。

> 本索引与技能 `SKILL.md` 的「核心工作流」章节一一对应；记忆侧负责"约束与为什么"（全局禁令章节 + 各工作流章节），技能侧负责"具体跑哪个脚本、怎么跑"，两者组合即可端到端落地任意 GitHub 操作。  
...
4. `gh` 能力总览覆盖的任务/事项/操作（见 `## 第8章 gh 能力速查`）。
...
- **多 remote 直观命名**：除 `origin`（你的 fork）+ `upstream`（原作者仓库）外，若同时关联公司 GitLab 等，可起直观名避免混淆：`git remote add company https://gitlab.company.com/项目.git`、`git remote add github https://github.com/你的账号/项目.git`。`origin` 只是习惯命名，非规定；合理多 remote 能让多仓库切换更清晰（Fork 仓库清单见 `### 7.2 Fork 仓库清单（账户资产索引）`）。
...
- **问题一** 本地 main ↔ 你的远端仓库(origin) 双向分叉：A–E 方案，暂停等确认（见 `### 12.2 第一步 — 本地 ↔ 你的远端仓库(origin)` 双向分叉）
...
> 改代码 / 提 PR 前，先过 **完整裁决器与全局契约面（从整个仓库代码整体性、全局性出发）**（画契约面 → 定方案 → 联动验证 → 文档代码同改），避免叠补丁 / 覆盖不全 / 过度覆盖违反契约。 完整纪律见 `## 第5章 代码编写、修改和BUG修复通用纪律`（通用，非 PR 专属）。
>
> **启动前必须先完成路径核验（ `## 第9章 路径核验`）。**
>
> 多任务并行开发（各任务独立工作树(worktree)、最终 `--no-ff` 普通合并回主线(main)）属本流程的并行特化变体，其专用 SOP 见 `## 第14章 工作流五 多工作树并行开发（–no-ff 普通合并特化）`。
...
> **前置约束（文档同步门禁依赖）**：本阶段下方的「文档—代码同步硬门禁」由 `github-personal-manager` 技能落地——**须先按 `## 第10章 工作流一 github-personal-manager 自动激活（GitHub 工作流总闸门，前置）` 激活该技能，在其技能目录下执行 `bash scripts/sop_docs_sync_check.sh`**（脚本与 `references/docs-sync-checklist.md` 由技能提供，不在本记忆内复制）。未激活技能时不得跳过该门禁，应改为「基于 `git diff`/`git status` 真实变化，人工核对 README/CHANGELOG/docs 是否同步」的等价自审。

- `git add <文件>` → `git commit -m "清晰描述，一 PR 一主题"`
- `git push -u origin feat/<topic>`
- ⚠️ **提交(commit)前硬门禁：文档—代码同步（分层检查清单）**（详见 `## 第15章 提交前文档同步门禁`）：先 `bash scripts/sop_docs_sync_check.sh <仓库路径>` 按 `references/docs-sync-checklist.md` 查仓库是否存在清单文件并分析同步状态；Tier 1（README/README_EN/CHANGELOG）未同步 → 必须基于 `git diff`/`git status` 真实变化更新相应章节并一并 `git add` 后再提交(commit)，严禁 Tier 1 未同步时直接提交(commit)代码；Tier 2（docs/·契约·i18n·examples 等）未同步须处理或说明；Tier 3（测试/锁文件）仅提示。
...
- 巡检（见 `## 第12章 工作流三 日常同步巡检`）遇工作区脏会硬停止——`stash` 是先"暂存半截"再继续的安全替代，但 stash 后仍需在合适时机 `pop` 回来继续。

---

## 第14章 工作流五 多工作树并行开发（–no-ff 普通合并特化）

> 适用：同一 Git 仓库需**多任务并行**开发（每个任务独立工作树(worktree)、代码隔离），最终**普通合并(–no-ff)**&#x56DE;主线(main)、补充变更文档(CHANGELOG)、按需清理工作树(worktree)与分支(branch)。本质是「标准代码修改（ `## 第13章 工作流四 标准代码修改`）」的**并行多工作树特化变体**：用 `git worktree` 突破单目录单工作树限制，用 `--no-ff` 强制双父合并碑(merge commit)以保留中间提交谱系、可整段回滚、不改写历史；回滚一律 `git revert`（禁用 `reset --hard`+强推，见 `## 第1章 全局禁令（最高优先级）`）。  
> 与既有规则关系：提交(commit)前文档—代码同步遵循 `## 第15章 提交前文档同步门禁`；分支(branch)清理的门禁与命令见本章阶段六（多工作树场景专用步骤）；改动前先过 **完整裁决器与全局契约面（避免叠补丁 / 覆盖不全 / 过度覆盖违反契约）**。 完整纪律见 `## 第5章 代码编写、修改和BUG修复通用纪律`（通用，非 PR 专属）。  
> **启动前必须先完成路径核验（ `## 第9章 路径核验`）：先用 `git rev-parse --show-toplevel` 等确认「要操作的仓库目录」到底是主仓库还是某个工作树(worktree)，路径异常（如指向非预期目录、根目录意外出现 `.git`）立即暂停、先问用户，对齐后再做。未核验路径不得进入任何后续步骤。**
...
- 先按 `## 第9章 路径核验` 确认 `<MAIN_REPO_PATH>` 真实存在且为 Git 仓库、确为预期目录；路径异常立即暂停、先问用户。
...
**守约定**：项目最高优先级约定区域功能完全保留（见 `### 14.4 核心原则（不可动摇）` 第5点）。
...
- **最高优先级约定区域冲突**（项目声明的核心模块）：**立即暂停，告知用户**，大白话说明，不擅自解（见 `### 14.4 核心原则（不可动摇）` 第5点）。

1. `git add <file>` 逐个标记已解决。
2. `git commit`（合并继续；可 `--no-edit` 或补正文）。

**合并碑结构验证（必须全满足，对照 `### 14.3 四大约束 ↔ 唯一验证矩阵`）**：
...
### 14.10 阶段五：清理工作树(worktree)（按要求 / 条件）
...
### 14.11 阶段六：清理分支(branch)（本地 + 远端，按要求 / 条件）

> 本节为 `## 第20章 工作流八 分支清理回收` 的多工作树场景专用步骤；通用分支清理的门禁与命令以该章为准。
...
### 14.13 验收闸门清单（四张，逐项打勾）

**合并前（ `### 14.8 阶段三：《--no-ff》 合并到主线(main)` 预检）**：
...
> **兜底说明（脚本化封装）**：下列手写命令模板的等价、可复制执行封装，由 `github-personal-manager` 技能的 `scripts/sop_worktree_add.sh` / `scripts/sop_worktree_merge.sh` / `scripts/sop_worktree_cleanup.sh` 提供，已内置本清单全部安全约束（`--no-ff` 双父碑合并、冲突预测暂停、合并校验防丢提交、本地远程跟踪引用清理 `git branch -d -r`、dry-run 优先、脏区硬停）。**优先直接调用脚本**；本清单作为原理参考与脚本不可用时的兜底，所有脚本约束与 `### 10.6 激活后执行点索引` 索引一致。
...
> 本模块记录 Fork 仓库 CI 实操中独有的坑与专属约束，作为本篇 GitHub 标准工作流中 CI / 发版类（工作流六/七）实操坑的实证溯源；已抽离账户名/邮箱，**统一用占位符 `<upstream>`/`<fork>`/`<feat>`/`<version>` 与「某仓库/某版本」表述**，确保可移植。涉及具体语言的坑（Rust/Node 等）已标注适用语言，不具备跨语言通用性。各主流程工作流已以「详见模块·本章第 X 点」单向指向此处（指向格式以速查索引表第二列为准）。详细展开见 `github-personal-manager` 技能 `references/fork-ci-pitfalls.md`。
...
> 触发时机：`gh pr checks` 或 `gh run list` 出现失败（红），需定位原因并修复回推。原则：定位与修复属本地/功能分支(feat) 动作，助手自动执行；凡改动 workflow 文件、重推标签(tag) 等影响面较大的动作，先大白话说明再执行。多数具体坑见 `## 第16章 Fork CI 实证要点`，本章只给排错主线。  
> 排错回到全局契约面裁决器：若 CI 红灯是「为过评审叠补丁」所致，先回全局契约面重画方案，而非继续串行打补丁。
>
> **启动前必须先完成路径核验（ `## 第9章 路径核验`）。**

### 17.1 第一步 — 定位失败的 run 与 job
...
### 17.2 第二步 — 按类型对号入座（详见本模块对应条目）

| 失败现象                              | 根因与去处                                               | 修复方向                                                                                         |
| --------------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| cargo fmt – --check 失败（Rust 项目适用） | 手写多行调用被 rustfmt 折叠（`## 第16章 Fork CI 实证要点` 第10点）     | 本地装 minimal + rustfmt 组件格式化，或靠临时 push-to-main CI 间接确认                                        |
| clippy -D warnings 报错（Rust 项目适用）  | 如 useless_conversion（`## 第16章 Fork CI 实证要点` 第9点）    | 在 功能分支(feat) 改，git push origin feat 重验                                                       |
| run 卡 action_required             | fork→上游 PR 审批闸门（`## 第16章 Fork CI 实证要点` 第1点）         | 等上游维护者 Approve；验证改走 fork 内部 push:[main]（第2点）                                                 |
...
>
> **单一事实源原则（强制）**：本工作流只定义"PR 全生命周期"的**编排顺序与 PR 专属要求**。凡路径核验、技能激活、本地↔origin↔upstream 对齐、开 PR 命令、文档门禁、CI 轮询、合并、分支清理等**他章已定义的命令/约束/步骤**，一律以「→ 第X章」方式**引用（调用）**，**不在本重复写**。禁止循环引用：本工作流引用他章，他章不反向引用本工作流（仅顶部速查索引表、`### 8.3 Pull Request（《gh pr》）` 索引作单向导航，不构成逻辑回环）。
>
> **启动前强制门禁（任何 PR 操作的第一步，不可跳过）**：
>
> 1. 先按 `## 第10章 工作流一 github-personal-manager 自动激活（GitHub 工作流总闸门，前置）` 激活 `github-personal-manager` 技能（GitHub 工作流总闸门）；
> 2. 再按 `## 第9章 路径核验` 完成路径核验（未核验路径不得进入任何后续步骤）；
> 3. 严守 `## 第1章 全局禁令（最高优先级）` 硬禁令（绝不强推/删 `origin/main` 等受保护分支；只推 `origin`，绝不推 `upstream`）。

### 18.1 阶段 0 — 前置门禁（引用，不重复定义）

- **技能激活**：→ `### 10.1 激活判定`–`### 10.6 激活后执行点索引`（含脚本索引与"路径无关"加载约定）。
- **路径核验**：→ `### 9.1 规则一：先核验"要操作的目录"再动手`–`### 9.4 规则四：发现异常先问，对齐后再做`（三步核验顺序、误报铁律、异常先问）。
- **硬禁令红线**：→ `### 1.1 禁止强推/删除自家 main（及受保护分支）`（禁强推/删 main）、`### 1.2 三段式二次授权铁律`（二次授权铁律）、`### 1.3 入库隐私闸门（2026-08-04 实测拦截后确立）`（入库隐私闸门）。
- **仓库三元组解析**：→ `### 10.6 激活后执行点索引` 的 `scripts/sop_resolve_repo.sh <仓库路径>`（提取 GH_USER/REPO_NAME/UPSTREAM）；或 `git remote -v` 自行确认 origin/upstream 指向。本工作流 `<upstream>` 一律从此解析（如 deepseek-pp 实为 `zhu1090093659/deepseek-pp`，非 `deepseek-ai`）。

### 18.2 阶段 1 — 开新 PR 前的三核验

开新 PR 前必须依次通过三道核验，任一不过则先修复/暂停，不进入开 PR。

**① 路径核验（引用）**：→ `## 第9章 路径核验` 。确认当前操作目录确为目标仓库根（`ls "<目录>/.git"` + `git -C "D:/..." rev-parse --show-toplevel`）。

**② 分支核验（PR 专属，聚焦）**：确认"将作为 PR 源分支(feature branch)"的状态正确：

- `git rev-parse --abbrev-ref HEAD` 确为预期 `feat/<topic>`（非 `main`、非游离 HEAD）；
- **born 检查**（防根提交异常，引用 `### 13.2 阶段 1 同步与建分支`）：`git rev-parse HEAD` 须解析出有效 SHA、有父提交；若 `unknown revision` → 先 `git reset --mixed main` 修复，绝不直接 `git add -A && git commit`；
- 分支已推到 你的远端仓库(origin)：`git rev-parse --abbrev-ref HEAD@{upstream}` 应为 `origin/feat/<topic>`；未推则先 `git push -u origin feat/<topic>`（引用 `### 13.3 阶段 2 提交 / 推送 / 触发 CI`）；
- 工作区干净（无未提交改动）：`git status --porcelain` 应为空；脏则先提交或 stash，否则不开 PR。

**③ 本地 ↔ origin ↔ upstream 对齐（引用）**：按 `### 12.2 第一步 — 本地 ↔ 你的远端仓库(origin)`–`### 12.3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)` 完成对齐，确保 PR 基于最新 `main`：

- 本地 main 与 origin/main：仅落后/领先→快进拉取/推送（引用 `### 12.2 第一步 — 本地 ↔ 你的远端仓库(origin)`）；
- origin/main 与 upstream/main：M>0,K>0 冲突→暂停；M=0,K>0→跟随上游（引用 `### 12.3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)`）；
...
### 18.3 阶段 2 — 重复 PR 检查（引用 + PR 专属口径）

开新 PR 前，先验是否已存在相同/重叠的开放 PR，避免重复开 PR。

- **查询口径（PR 专属，引用 `### 12.3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)`）**：一律用**作者口径**，弃用 `--head` 窄口径：

```bash
gh pr list --repo <upstream> --author zhangweildlh --state all
```

- ⚠️ 口径区别（引用 `### 12.3 第二步 — 你的远端仓库(origin/fork) ↔ 上游仓库(upstream)`）：`--head zhangweildlh:main` 只匹配源分支恰叫 `main` 的 PR，会漏掉 `feat/*`/`fix/*` 分支的 PR；`--author zhangweildlh` 按作者匹配，返回我开的全部 PR，不漏判。仅在查"main→main 通道"时才叠加 `--head`。
- **判定**：
- 已有 open PR 覆盖同一改动 → **不重复开**，直接在那条 PR 上更新（追加提交/编辑正文）；
- 已有 rejected（closed 未合并）PR → 视为 `### 12.4 冲突处理（六类问题）` 问题四，按反馈改后开新 PR（暂停等指令）；
- 无 PR → 进入阶段 3/4 开新 PR。

### 18.4 阶段 3 — 查询并遵循上游仓库对 PR 的要求与规范（PR 专属，新内容）

开 PR 前必须先用 `gh` 读取**上游仓库(upstream)** 的 PR 规范与 CI 门槛（引用 `## 第11章 工作流二 信息读取与搜索` gh 优先原则），确保 PR 正文与改动满足上游验收。
...
- 解码：GitHub `contents` API 返 base64，Git Bash 下用 `base64 -d` 解码（引用 `### 11.4 跨命令通用约束`）；`gh` 不可用/无权限时回退 WebFetch（引用 `### 11.2 优先顺序（硬规则）` 回退条件）。
...
→ 推论：向该上游开 PR 时，zhangweildlh 是**外部贡献者**（非仓库 owner），user-facing PR **必须**在 `Local Feature Evidence` 段附截图，否则 CI 红灯。此即本章 ⑤.5「截图与上传」的由来。

### 18.5 阶段 4 — 开新 PR 的规范操作（引用 + PR 专属补充）

- **开 PR 命令（引用 `### 13.3 阶段 2 提交 / 推送 / 触发 CI`）**：
- 向 上游仓库(upstream) 贡献：`gh pr create --repo <upstream> --head zhangweildlh:feat/<topic> --base main`
- fork 内部 PR（触发 fork CI）：`gh pr create --repo zhangweildlh/<fork> --head zhangweildlh:feat/<topic> --base main`
- ⚠️ 多 remote 口径坑（引用 `### 13.3 阶段 2 提交 / 推送 / 触发 CI`）：一律显式带 `--repo`，避免默认取 upstream 报 "No commits between…"。
- ⚠️ PR 正文用 `--body-file`（引用 `### 13.3 阶段 2 提交 / 推送 / 触发 CI`）：先写正文到文件再 `gh pr create --body-file <file>`，避免中文括号被 bash 解析失败。
- **PR 正文必须包含（引用 `### 18.4 阶段 3 — 查询并遵循上游仓库对 PR 的要求与规范（PR 专属，新内容）` 上游规范）**：
...
- 显式声明本次**契约边界**（覆盖到哪、不覆盖到哪、剩余边界为何）——呼应 `## 第5章 代码编写、修改和BUG修复通用纪律` 分阶段手册「提交 PR 时」的 PR 描述要求；
- user-facing 改动：在 Evidence 段附截图（方法见 阶段 5 ⑤.5）。
- **文档同步门禁（引用 `## 第15章 提交前文档同步门禁` + `### 10.6 激活后执行点索引` 的 `sop_docs_sync_check.sh` 脚本索引）**：提交前 Tier 1（README/CHANGELOG）必须同步，未同步不得直接提交/开 PR。
- **触发并轮询 CI（引用 `### 8.3 Pull Request（《gh pr》）` 的 `gh pr checks` / `gh run list` + `### 10.6 激活后执行点索引` 的 `sop_pr_checks.sh` 轮询脚本）**：`gh pr checks` / `gh run list` 必须全绿；CI 触发条件以各仓库 `.github/workflows/` 为准（引用 `### 13.3 阶段 2 提交 / 推送 / 触发 CI` 注）。

### 18.6 阶段 5 — PR 审查意见回应（核心，含多轮）（PR 专属，新内容）

> 本章 PR 审查意见回应相关的代码改动，其通用标准见 `## 第5章 代码编写、修改和BUG修复通用纪律`（通用，非 PR 专属）；本章仅承接其精神、不重述。

收到评审（review comment / review request-changes / inline comment）后，按以下顺序回应。**先对齐、再查实、后修改、再回答、最后截图与上传**；多轮迭代回到本阶段开头整体重画，绝不叠补丁。

**⑤.1 拉取并核对评审意见（引用 `### 8.3 Pull Request（《gh pr》）` 命令）**：
...
- 区分三类意见：**A 真问题（须改）/ B 边界澄清（文档即可）/ C 误判（用裁决器驳回）**（呼应 `## 第5章 代码编写、修改和BUG修复通用纪律` 分阶段手册「收到评审回复时」的分类）。
...
- 分支须已推 origin 且与 PR 关联（引用 `### 18.2 阶段 1 — 开新 PR 前的三核验` ②）。
...
- **整体核对**：将本条评审与 `### 18.2 阶段 1 — 开新 PR 前的三核验` / `### 18.3 阶段 2 — 重复 PR 检查（引用 + PR 专属口径）` / `### 18.4 阶段 3 — 查询并遵循上游仓库对 PR 的要求与规范（PR 专属，新内容）` 已确认的契约面、其余评审项耦合一起看，避免"只盯这一条"。

**⑤.4 文案回答（引用 `## 第5章 代码编写、修改和BUG修复通用纪律` 分阶段手册「收到评审回复时」精神 + PR 专属）**：

- 每条评审逐条回应，引用**裁决/契约结论**（为何扩/缩/保契约），不空泛认错；
- A 类：说明改了哪、如何验证；B 类：文档澄清即可，附链接/段落；C 类（误判）：用全局契约面/裁决器依据**有理有据驳回**，不硬改；
- 多轮：同一 diff 内一次性收口所有 A 类 + 联动项，不在上一轮补丁上再叠（呼应 `## 第5章 代码编写、修改和BUG修复通用纪律` 分阶段手册「收到评审回复时 / 多次复提交」）。

**⑤.5 截图与上传（user-facing 证据，PR 专属）**：
...
**⑤.6 推送更新 PR（引用 `### 13.3 阶段 2 提交 / 推送 / 触发 CI` 提交/推送）**：
...
- 每轮回到 ⑤.1，整体重画方案后同 diff 处理（不叠补丁，引用 `## 第5章 代码编写、修改和BUG修复通用纪律` 分阶段手册「多次复提交」）；
- 若发现上轮越界/漏点，在同分支 amend/squash 收口，保持 PR 单一连贯。

### 18.7 阶段 6 — PR 合并（引用）

- 贡献 upstream：由上游维护者合并，助手仅监控，不得自行合并（硬约束）；
- 自有仓库/自测 PR：`gh pr merge`（引用 `### 8.3 Pull Request（《gh pr》）`）；
- fork 内部 PR（仅触发 CI）：`gh pr merge --squash`；
- 合并前冲突/分支保护：引用 `## 第14章 工作流五 多工作树并行开发（–no-ff 普通合并特化）`（多工作树）或 `### 12.4 冲突处理（六类问题）`（冲突决策树）。

### 18.8 阶段 7 — 其他 PR 操作（引用 `### 8.3 Pull Request（《gh pr》）`，补用法）
...
- 作为评审人审他人 PR：`gh pr review <编号> --approve | --request-changes | --comment`（引用 `### 8.3 Pull Request（《gh pr》）`）；
- 评论互动：`gh pr comment <编号> --body "..."`（含证据图链接，见 `### 18.6 阶段 5 — PR 审查意见回应（核心，含多轮）（PR 专属，新内容）` ⑤.5）。
...
### 18.10 强门禁总述（PR 全生命周期）

- 仅「路径核验通过 + 分支/对齐通过 + 重复 PR 已排除 + 上游规范已遵循 + 正文合规 + CI 全绿」的 PR 可开/可合；
- 一切冲突、公开动作（强推 feat 需 `--force-with-lease` 二次确认、合并受保护分支走 PR、删分支前 PR 状态核验）一律大白话 + 后果 + 暂停等指令（呼应 `### 10.6 激活后执行点索引` 强门禁）；
...
> 触发时机：需要构建二进制产物或正式发版时。**硬前提**：fork 发版仅用于自取构建产物，**绝不**发布到 crates.io / PyPI（见 `## 第16章 Fork CI 实证要点` 第6点）。打标签(tag) 会触发 CI 并生成 Release，属公开动作 → 先说明将推的版本，暂停等指令。
>
> **启动前必须先完成路径核验（ `## 第9章 路径核验`）。**

### 19.1 第一步 — 发版前检查（缺一不可）

- CHANGELOG 顶部有对应 `[X.Y.Z] - <date>` 版本段（`## 第16章 Fork CI 实证要点` 第7点），否则 `create-release` 抽 notes 失败。
- `release.yml` 中**发布类 job**（如 `publish-to-crates-io`/`pypi`/`build-python-wheels` 等，取决于该仓库所用语言与发布通道）已加 `if: github.repository == '<upstream>'` 守卫（fork 上求值 false 自动跳过、上游上 true 正常发布），仅保留 `create-release` + `build-release`（`## 第16章 Fork CI 实证要点` 第6点）。⚠️ 禁止写成纯常量 `if: false`（actionlint 拒绝）。
- 未勾 "Require actions pinned to full-length SHA"（`## 第16章 Fork CI 实证要点` 第8点）。
...
- 需重新触发（同版本重跑）：先删远端标签(tag) 再重推 → `git push origin :refs/tags/v<version>` → `git push origin v<version>`（见 `## 第16章 Fork CI 实证要点` 第3点）。**禁止用 `git push --force-with-lease origin v<version>` 强推标签**，以免与"禁止强推 main"禁令混淆。
...
> **启动前必须先完成路径核验（ `## 第9章 路径核验`）。**
>
> 多工作树(worktree) 场景的分支清理专用步骤（含工作树(worktree)失效游离的 `rm -rf` 处置）见 `### 14.11 阶段六：清理分支(branch)（本地 + 远端，按要求 / 条件）`。
...
### 21.2 清理工区工作流（强门禁：先搜列、后确认、再删除）

1. **搜索并详细列出（只读）**：按 `### 21.1 可删除文件定义（清理工区时的目标集）` 定义，在你指定的工区（默认当前工作目录）搜索全部可删除文件，**逐项列出完整路径 + 大小 + 修改时间 + 类型归属**，生成明细清单呈现给你；此步不删除任何文件。
...
- 明确保留：`CHANGELOG.md`（标准仓库版本记录，**非**"阶段性报告"，不按 `### 21.1 可删除文件定义（清理工区时的目标集）` 处理）
- **受保护（禁止清理）**：
- `.git/`：版本库本体，删则丢失全部提交历史，永不碰
- `.workbuddy/`：项目记忆库（含 `MEMORY.md` 与 `memory/` 日志），禁止删除
- **边界澄清（避免歧义）**：`tests/`、`scripts/`、`desktop/` 作为仓库**常驻代码目录**整体保留；其中**临时验证用**的测试文件 / 脚本仍按 `### 21.1 可删除文件定义（清理工区时的目标集）` 判定为可删除（排除冒烟测试），即"目录保留、目录内临时文件可删"。`CHANGELOG.md` 与 `### 21.1 可删除文件定义（清理工区时的目标集）` 「阶段性报告」互斥，前者永远保留。
...
## 第22章 常用跨项目技能（常驻工具链，跨项目）

> 经多会话实证、跨项目复用的常驻技能；与 git/gh 流程互补（路径核验另见 `## 第9章 路径核验`），涉及对应场景时主动加载。
>
...
| github-personal-manager（GitHub 管理统一执行） | 提交 / 推送 / 开 PR / 同步 / 发版 / 分支清理等本地与远端 GitHub 操作的一站式技能 | 凡涉及 GitHub 仓库/项目/代码操作，必须先按名称激活此技能（详见 `## 第10章 工作流一 github-personal-manager 自动激活（GitHub 工作流总闸门，前置）`）      |
