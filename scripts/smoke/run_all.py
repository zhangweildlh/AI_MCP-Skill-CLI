...
用法：
  python scripts/smoke/run_all.py                 # 运行全部分层
  python scripts/smoke/run_all.py --tier 0,1      # 仅运行 Tier0 + Tier1（预提交钩子场景）
  python scripts/smoke/run_all.py --tier 0,1,2,3  # CI 场景（不含行为级 Tier4）
  python scripts/smoke/run_all.py --list          # 仅列出发现的 Skill
  python scripts/smoke/run_all.py --strict        # WARN 也视为失败
  python scripts/smoke/run_all.py --json out.json # 输出 JSON 报告
