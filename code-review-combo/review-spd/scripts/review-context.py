...
PATH_FILTER = ""  # subdirectory (relative to git root) supplied via --path


def _scope() -> list[str]:
    """Restrict git commands to PATH_FILTER when set (subdirectory review)."""
    return ["--", PATH_FILTER] if PATH_FILTER else []
...
def status_porcelain() -> str:
    args = ["status", "--porcelain=v1", "-uall", *_scope()]
    return git_stdout(args, check=False)
...
def changed_files_for_range(base: str, head: str) -> str:
    return git_stdout(["diff", "--name-status", "--find-renames", base, head, *_scope()], check=False)


def diff_stat_for_range(base: str, head: str) -> str:
    return git_stdout(["diff", "--stat", "--find-renames", base, head, *_scope()], check=False)


def diff_for_range(base: str, head: str) -> str:
    return git_stdout(["diff", "--find-renames", base, head, *_scope()], check=False)
...
def collect_uncommitted() -> dict[str, str]:
    ...
    tracked_stat = git_stdout(["diff", "--stat", "--find-renames", "HEAD", *_scope()], check=False)
    tracked_files = git_stdout(["diff", "--name-status", "--find-renames", "HEAD", *_scope()], check=False)
    tracked_diff = git_stdout(["diff", "--find-renames", "HEAD", *_scope()], check=False)
...
def collect_branch(branch: str, base_arg: str | None) -> dict[str, str]:
    ...
    commits = git_stdout(
        ["log", "--reverse", "--format=%H%x09%h%x09%ad%x09%s", "--date=short", f"{merge_base}..{branch}", *_scope()],
        check=False,
    )
    return {
        "mode": "branch",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--path", help="Subdirectory (relative to git root) to limit the review scope to; without it the whole repository is scanned.")
